...
Metadata providing UI hints can be specified either xref:ug:vw:ugvw.adoc#annotation-based[using annotations], or using an `Xxx.layout.xml` file (where `Xxx` is the entity or view model object to be rendered).
...
* UI hints can be provided for xref:ug:fun:ugfun.adoc#contributed-members[contributed associations and actions] that are synthesised at runtime.

It is also possible to download an initial `.layout.xml` - capturing any existing layout metadata - using the xref:rg:svc:rgsvc.adoc#_rgsvc_metadata-api_LayoutService[`LayoutService`] (exposed on the prototyping menu) or using a xref:rg:cms:rgcms.adoc#Object[mixin action] contributed to every domain object.
...
The `Xxx.layout.xml` file is just the serialized form of a xref:rg:cms:rgcms.adoc#layout[`Grid`] layout class defined within Apache Isis' applib.
...
One way in which the domain object can specify an alternate layout is through its xref:rgcms.adoc#layout[`layout()`] method.
...
More information about these classes can be found in xref:rg:cms:rgcms.adoc#layout[the reference guide].  More information on Bootstrap 3's grid system can be found link:http://getbootstrap.com/css/#grid[here].
...
This topic is discussed in more detail xref:ug:fun:ugfun.adoc#unreferenced[below].
...
Note also the `unreferencedProperties` attribute for the fieldset; this topic is discussed in more detail xref:ug:fun:ugfun.adoc#unreferenced[below].
...
As noted earlier on, it is possible to download layout XML files using the xref:rg:svc:rgsvc.adoc#LayoutService[`LayoutService`] (exposed on the prototyping menu); this will download a ZIP file of layout XML files for all domain entities and view models.
Alternatively the layout XML for a single domain object can be downloaded using the xref:rg:cms:rgcms.adoc#Object[mixin action] (contributed to every domain object).
...
| xref:rg:ant:rgant.adoc#_rgant-ActionLayout[`@ActionLayout`], xref:rg:ant:rgant.adoc#rgant-CollectionLayout[`@CollectionLayout`]
...
* xref:rg:svc:rgsvc.adoc#_rgsvc_metadata-api_LayoutService[`LayoutService`] (whose functionality is exposed on the prototyping menu as an action) and lso the a xref:rg:cms:rgcms.adoc#Object[mixin action]

* xref:rg:svc:rgsvc.adoc#_rgsvc_presentation-layer-spi_GridService[`GridService`] and its supporting services, xref:rg:svc:rgsvc.adoc#_rgsvc_presentation-layer-spi_GridLoaderService[`GridLoaderService`] and xref:rg:svc:rgsvc.adoc#GridSystemService[`GridSystemService`]

* xref:rg:cms:rgcms.adoc#layout[grid layout classes], defined in the Apache Isis applib
...
If using an Apache Isis xref:ug:fun:ugfun.adoc#_ugfun_getting-started_helloworld-archetype[HelloWorld] xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp] archetypes, then the POM is already correctly configured.
