...
In the vast majority of cases customization should be sufficient by xref:ug:vw:ugvw.adoc#replacing-page-elements[replacing elements of a page].
...
| SIGN_UP
| The sign-up page (if xref:rg:svc:rgsvc.adoc#UserRegistrationService[user registration] is enabled).

| SIGN_UP_VERIFY
| The sign-up verification page (if xref:rg:svc:rgsvc.adoc#UserRegistrationService[user registration] is enabled; as accessed by link from verification email)
