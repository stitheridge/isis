...
* the xref:rg:svc:rgsvc.adoc#UserRegistrationService[user registration service], which provides an API to create the user account
* the xref:rg:svc:rgsvc.adoc#EmailNotificationService[email notification service], which provides an API for to send the verification emails
* the xref:rg:svc:rgsvc.adoc#EmailService[email service], that is used by the email notification service to actually send the email.
...
* register an implementation of the xref:rg:svc:rgsvc.adoc#UserRegistrationService[user registration service] (eg by using the (non-ASF) link:http://platform.incode.org[Incode Platform^]'s security module)

* configure the xref:rg:svc:rgsvc.adoc#EmailService[email service]
...
It is also possible to configure the Wicket viewer to suppress the sign-up page link and/or the password reset page, see xref:ug:vw:ugvw.adoc#sign-in[here] for further details.
