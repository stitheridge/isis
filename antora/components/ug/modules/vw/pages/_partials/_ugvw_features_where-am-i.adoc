...
The navigable tree-structure, as provided by the 'Where am I' feature, is declared at compile-time (predefined by the programmer), whereas related features (xref:ug:vw:ugvw.adoc#_ugvw_features_recent-pages[Recent Pages] and xref:ug:vw:ugvw.adoc#bookmarked-pages[Bookmarked Pages]) are populated at runtime only after user interaction.
