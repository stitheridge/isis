...
Further discussion xref:ugvw.adoc#remember-me[below].
...
Further discussion xref:ugvw.adoc#password-reset[below].
...
Further discussion xref:ugvw.adoc#sign-up[below].
...
See also the xref:ugvw.adoc#_ugvw_configuration-properties_bookmarks-and-breadcrumbs[bookmarks and breadcrumbs] and xref:ugvw.adoc#themes
...
|`hidden`, `table` +
(`hidden`)
|Default for the default view for all (parented) collections if not explicitly specified using xref:rg:ant:rgant.adoc#defaultView[`@CollectionLayout#defaultView()`]

By default the framework renders (parented) collections as "hidden", ie collapsed.
These can be overridden on a case-by-case basis using the xref:rg:ant:rgant.adoc#defaultView[`@CollectionLayout#defaultView()`] or the corresponding `<collectionLayout defaultView="...">` element in the `Xxx.layout.xml` layout file.
...
|`TOP`, `LEFT` +
(`LEFT`)
|Default for label position for all properties if not explicitly specified using xref:rg:ant:rgant.adoc#labelPosition[`@PropertyLayout#labelPosition()`]
...
The property can be overridden on a property-by-property basis using xref:rg:ant:rgant.adoc#_rgant-PropertyLayout_promptStyle[`@Property#promptStyle()`]) or  xref:rg:ant:rgant.adoc#promptStyle[`@Action#promptStyle()`]).
...
See the discussion on the xref:ug:vw:ugvw.adoc#sidebar-vs-modal-dialogs[sidebar vs modal dialogs] feature for further details.
...
The Wicket viewer has a xref:ug:vw:ugvw.adoc#titles-in-tables[mechanism to automatically shorten] the titles of objects specified using `@Title`.
...
[TIP]
====
You can also develop and install a custom themes (eg to fit your company's look-n-feel/interface guidelines); see the xref:ug:vw:ugvw.adoc#custom-bootstrap-theme[Extending] chapter for further details.
====
...
`liveReloadUrl`
| URL
|Specifies the URL if xref:support:dg:dg.adoc#gradle-liveReload[live reload] is set up, eg: +
...
| `true`,`false` +
(`_true_`)
| To disable the xref:ugvw.adoc#where-am-i["Where am I"] feature.
