...
[TIP]
====
Both the xref:ug:fun:ugfun.adoc#_ugfun_getting-started_helloworld-archetype[HelloWorld] and xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp] archetypes are pre-configured to use Apache Shiro, so much of what follows may well have been set up already.
====


== Telling Apache Isis to use Shiro

To tell Apache Isis to use Shiro when using an xref:rg:cms:rgcms.adoc#AppManifestAbstract[`AppManifestAbstract.BUILDER`], simply specify the "authMechanism" as "shiro".

For example, the xref:ug:fun:ugfun.adoc#helloworld-archetype[HelloWorld archetype] bootstraps using:
