...
[TIP]
====
The sign-in page to render is pluggable; see xref:ug:vw:ugvw.adoc#custom-pages[extensions chapter] for details.
====
...
These pages all "reach inside" Apache Isis framework using a mechanism similar to xref:ug:btb:ugbtb.adoc#headless-access[Headless access] in order to actually do their stuff.

[TIP]
====
The sign-in verification page to render is pluggable; see xref:ug:vw:ugvw.adoc#custom-pages[extensions chapter] for details.
====

User registration is only available if the xref:rg:svc:rgsvc.adoc#UserRegistrationService[`UserRegistrationService`] is configured; this is used by the framework to actually create new instances of the user as accessed by the corresponding (Shiro) realm.
