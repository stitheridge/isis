...
As well as defining a xref:ug:fun:ugfun.adoc#metamodel[metamodel] of the structure (domain classes) of its domain objects, Apache Isis also manages the runtime instances of said domain objects.
...
* as a handle to any object in an audit record, as used by xref:rg:svc:rgsvc.adoc#AuditerService[`AuditerService`];

* similarly within implementations of xref:rg:svc:rgsvc.adoc#CommandService[`CommandService`] to persist `Command`
objects

* similarly within implementations of xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`]
...
These are represented as `Bookmark`s, obtained from the xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`].
Deep links meanwhile can be obtained from the xref:rg:svc:rgsvc.adoc#DeepLinkService[`DeepLinkService`].

OIDs can also be converted into XML format, useful for integration scenarios.
The xref:rg:cms:rgcms.adoc#schema-common[common schema] XSD defines the `oidDto` complex type for precisely this purpose.
