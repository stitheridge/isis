...
This repository calls the framework-provided xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`] to actually submit the query.
...
----
<1>	The xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`] is a generic facade over the JDO/DataNucleus API.
...
When an object is loaded from the database the framework will emit `ObjectLoadedEvent` xref:ug:fun:ugfun.adoc#lifecycle-events[lifecycle event].


== Type-safe queries

DataNucleus also supports type-safe queries; see xref:rg:svc:rgsvc.adoc#type-safe-jdoql-queries[here] for further details.
