...
+
Note that this requires that the object is instantiated using the framework, see xref:ug:fun:ugfun.adoc#instantiating[here] for further discussion
...
For example, if annotated with xref:rg:ant:rgant.adoc#updatingLifecycleEvent[`@DomainObjectLayout#updatingLifecycleEvent()`], the appropriate (subclass of) `ObjectUpdatingEvent` will be emitted.
