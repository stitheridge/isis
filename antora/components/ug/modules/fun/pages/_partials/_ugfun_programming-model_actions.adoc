...
While xref:ug:fun:ugfun.adoc#_ugfun_programming-model_properties[properties] and xref:ug:fun:ugfun.adoc#collections[collections] define the state held by a domain object (its "know what" responsibilities), actions define the object's behaviour (its "know how-to" responsibilities).
...
This is a slight simplification; there are a number of other method prefixes (such as `hide` or `validate`) that represent xref:ug:fun:ugfun.adoc#business-rules[business rules]); these also not treated as actions.
...
This is done using either a supporting xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_choices[`choices`] or xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete`] method.
...
As the example suggests, any collection parameter type must provide a way to select items, either by way of a "choices" or "autoComplete" supporting method or alternatively defined globally using xref:rg:ant:rgant.adoc#_rgant-DomainObject[`@DomainObject`] on the referenced type (described xref:ug:fun:ugfun.adoc#removing-boilerplate[above]).


== Optional Parameters

Whereas the xref:ug:fun:ugfun.adoc#__ugfun_programming-model_properties_optional-properties[optionality of properties] is defined using xref:rg:ant:rgant.adoc#allowsNull[`@javax.jdo.annotations.Column#allowsNull()`], that JDO annotation cannot be applied to parameter types.
Instead, either the xref:rg:ant:rgant.adoc#_rgant-Nullable[`@Nullable`] annotation or the xref:rg:ant:rgant.adoc#optionality[`@Parameter#optionality()`]  annotation/attribute is used.
...
See also xref:ug:fun:ugfun.adoc#properties-vs-parameters[properties vs parameters].

== ``String`` Parameters (Length)

Whereas the xref:ug:fun:ugfun.adoc#__ugfun_programming-model_properties_datatypes_strings[length of string properties] is defined using xref:rg:ant:rgant.adoc#length[`@javax.jdo.annotations.Column#length()`], that JDO annotation cannot be applied to parameter types.
Instead, the xref:rg:ant:rgant.adoc#maxLength[`@Parameter#maxLength()`] annotation/attribute is used.
...
----
<1> Specifies the property length using the JDO xref:rg:ant:rgant.adoc#length[`@Column#length()`] annotation
<2> Specifies the parameter length using the (Apache Isis) xref:rg:ant:rgant.adoc#maxLength[`@Parameter#maxLength()`] annotation
...
Whereas the xref:ug:fun:ugfun.adoc#__ugfun_programming-model_properties_datatypes_bigdecimals[precision of BigDecimal properties] is defined using xref:rg:ant:rgant.adoc#scale[`@javax.jdo.annotations.Column#scale()`], that JDO annotation cannot be applied to parameter types.
...
----
<1> Specifies the property precision using xref:rg:ant:rgant.adoc#scale[`@Column#scale()`]
