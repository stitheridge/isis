...
In conjunction with xref:ug:vw:ugvw.adoc#tweaking-css-classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.
...
Alternatively, the title can be provided simply by implementing the xref:rg:cms:rgcms.adoc#title[`title()`] reserved method.
...
A variation on this approach also supports localized names; see xref:ug:btb:ugbtb.adoc#i18n[beyond-the-basics] guide for further details.
...
In the target object, we define an appropriate event type and use the xref:rg:ant:rgant.adoc#titleUiEvent[`@DomainObjectLayout#titleUiEvent()`] attribute to specify:
...
This is specified using the xref:rg:ant:rgant.adoc#cssClassFa[`@DomainObjectLayout#cssClassFa()`] attribute.
...
To customise the icon on an instance-by-instance basis, we implement the reserved xref:rg:cms:rgcms.adoc#iconName[`iconName()`] method.
...
In the target object, we define an appropriate event type and use the xref:rg:ant:rgant.adoc#iconUiEvent[`@DomainObjectLayout#iconUiEvent()`] attribute to specify.
...
It is also possible for an object to return a xref:rg:cms:rgcms.adoc#cssClass[CSS class].
In conjunction with xref:ug:vw:ugvw.adoc#tweaking-css-classes[customized CSS] this can be used to apply arbitrary styling; for example each object could be rendered in a page with a different background colour.
...
xref:rg:ant:rgant.adoc#cssClass[`@DomainObjectLayout#cssClass()`].
...
To customise the icon on an instance-by-instance basis, we implement the reserved xref:rg:cms:rgcms.adoc#cssClass[`cssClass()`] method.
...
In the target object, we define an appropriate event type and use the xref:rg:ant:rgant.adoc#cssClassUiEvent[`@DomainObjectLayout#cssClassUiEvent()`] attribute to specify.
