...
An appropriate implementation of xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] can then be used to return the appropriate DTO (as XML).

For the latter case, one design is simply for the application to instantiate the view model, then call the xref:rg:svc:rgsvc.adoc#JaxbService[`JaxbService`] to obtain its corresponding XML.  This can then be published onto the ESB, for example using an http://activemq.apache.org[Apache ActiveMQ (TM)] queue.
...
It provides an implementation of the xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-spi_PublisherService[`PublisherService`], but which simply publishes instances of xref:rg:cms:rgcms.adoc#schema-ixn[`InteractionDto`] to an ActiveMQ queue.
