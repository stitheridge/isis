...
One option is to use the xref:rg:ant:rgant.adoc#cssClassFa[`@ActionLayout#cssClassFa()`].
For example:
...
Alternatively, you can specify these hints dynamically in the xref:ug:vw:ugvw.adoc#layout[`Xxx.layout.xml`] for the entity:
...
Rather than annotating every action with xref:rg:ant:rgant.adoc#_rgant-ActionLayout_cssClassFa[`@ActionLayout#cssClassFa()`] and xref:rg:ant:rgant.adoc#cssClass[`@ActionLayout#cssClass()`] you can instead specify the UI hint globally using regular expressions.
Not only does this save a lot of boilerplate/editing, it helps ensure consistency across all actions.

To declare fa classes globally, use the xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] `isis.reflector.facet.cssClassFa.patterns` (a comma separated list of key:value pairs).
...
* xref:rg:ant:rgant.adoc#cssClass[`@ActionLayout#cssClass()`] for actions

* xref:rg:ant:rgant.adoc#cssClass[`@PropertyLayout#cssClass()`] for properties, and

* xref:rg:ant:rgant.adoc#cssClass[`@CollectionLayout#cssClass()`] for collections.
...
It is also possible to specify CSS classes globally, using the xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] `isis.reflector.facet.cssClass.patterns` configuration property.
