...
[WARNING]
====
As was explained xref:ug:fun:ugfun.adoc#view-models[earlier], the approach is described here is neither as flexible nor as powerful as using the JAXB-style of view models.
...
----
|Annotated with xref:rg:ant:rgant.adoc#_rgant-DomainObject_nature[`@DomainObject#nature()`] and a nature of `EXTERNAL_ENTITY`, with memento derived automatically from the properties of the domain object.  Collections are ignored, as are any properties annotated as xref:rg:ant:rgant.adoc#notPersisted[not persisted].
...
----
|Implement xref:rg:cms:rgcms.adoc#ViewModel[`ViewModel`] interface.  The memento is as defined by the
