...
Both domain entities and view models can be instantiated using the xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`] provided by the framework.
For example:
...
When the framework instantiates the object, all services are injected into the framework, and an `ObjectCreatedEvent` xref:ug:fun:ugfun.adoc#lifecycle-events[lifecycle event] will also be emitted.
...
Instead the xref:rg:svc:rgsvc.adoc#ServiceRegistry[`ServiceRegistry`] service can be used to inject services:
