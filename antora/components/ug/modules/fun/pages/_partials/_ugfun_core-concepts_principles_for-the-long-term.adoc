...
This technique is also used extensively by the framework itself.
 For example, the xref:rg:svc:rgsvc.adoc#_rgsvc_persistence-layer-spi_AuditerService[`AuditerService`] SPI enables custom auditing, and the xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`] SPI enables custom publishing
