...
This is done using the xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`].
For example:
...
This is then executed either at the end of the transaction, or if a xref:ug:fun:ugfun.adoc#_ugfun_crud_finding[query is run], or if the internal queue is manually flushed using xref:rg:svc:rgsvc.adoc#TransactionService[`TransactionService`]'s `flush()` method.
...
When an object is persisted the framework will emit `ObjectPersistingEvent` and `ObjectPersistedEvent` xref:ug:fun:ugfun.adoc#lifecycle-events[lifecycle events].
