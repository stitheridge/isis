...
If the list of available options is fixed then the developer can provided a list a xref:rg:cms:rgcms.adoc#choices[`choices...()`] supporting method (for either and action parameter or when editing a property).
These are rendered in a drop-down.

If the list of available options is much larger, then the developer can use an xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete...()`] supporting method.
...
The developer indicates this using a xref:rg:cms:rgcms.adoc#default[`default...()`] supporting method.
...
For more on this, see xref:rg:ant:rgant.adoc#bounding[`@DomainObject#bounding()`].
...
For more on this, see xref:rg:ant:rgant.adoc#autoCompleteRepository[`@DomainObject#autoCompleteRepository()`].
