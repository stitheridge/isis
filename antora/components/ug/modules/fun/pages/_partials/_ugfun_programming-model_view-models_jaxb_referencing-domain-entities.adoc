...
However, the identity of the underlying entity can be well defined; Apache Isis defines the xref:rg:cms:rgcms.adoc#schema-common[common schema] which defines the `<oid-dto>` element (and corresponding `OidDto` class): the object's type and its identifier.
...
And this class is similar to the xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`]: it knows how to create an `OidDto` from an object reference.
...
This capability is particularly useful when the JAXB view model is being used as a xref:ug:fun:ugfun.adoc#dto[DTO].
