...
In Apache Isis every business action (and indeed, property and collection) emits domain events through the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_EventBusService[`EventBusService`], and can optionally also be published through the xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`].
The former are dispatched and consumed in-process and within the same transaction, and for this reason the xref:rg:cms:rgcms.adoc#AbstractSubscriber[subscribers] can also veto the events.
