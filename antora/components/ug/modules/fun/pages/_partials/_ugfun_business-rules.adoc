...
One decoupled approach is using xref:ug:fun:ugfun.adoc#domain-events[domain events]; the checks above correspond to the phases of the domain event.
...
For more information, see the xref:rg:cms:rgcms.adoc#hide[`hide...()`] section in the appropriate reference guide.
...
For more information, see  xref:rg:cms:rgcms.adoc#disable[`disable...()`] section in the appropriate reference guide.
...
This can be made a global policy using a xref:rg:cfg:rgcfg.adoc#isis-objects-editing[configuration setting]:
...
For more information, see the xref:rg:cms:rgcms.adoc#validate[`validate...()`] section in the appropriate reference guide.
The reference guide also explains how to define validation declaratively, using the xref:rg:ant:rgant.adoc#_rgant-Parameter_mustSatisfy[`@Parameter#mustSatisfy()`] or xref:rg:ant:rgant.adoc#mustSatisfy[`@Property#mustSatisfy()`] attributes.
...
[NOTE]
====
The framework also allows side-effect code to be placed in separate xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_modify[`modify...()`], xref:rg:cms:rgcms.adoc#clear[`clear...()`] supporting methods; if present then these will be called by the framework rather than the setter.
