...
Notably the metamodel is xref:ug:btb:ugbtb.adoc#programming-model[extensible]; it is possible to teach Apache Isis new programming conventions/rules (and conversely to remove those that are built in).
...
xref:ug:vw:ugvw.adoc#layout[layout hints], such as `Customer.layout.xml`.
