...
It also provides a `@CollectionLayout` for UI hints (though the information in this annotation may instead be provided by a supplementary xref:ug:vw:ugvw.adoc#layout[`.layout.xml`] file
...
Unlike xref:ug:fun:ugfun.adoc#properties[properties], the framework (at least, the xref:ug:vw:ugvw.adoc#[Wicket viewer]) does not allow collections to be "edited".
Instead, xref:ug:fun:ugfun.adoc#actions[action]s can be written that will modify the contents of the collection as a side-effect.
...
If you want to visualize an array of value types in Apache Isis, then one option is to wrap value in a view model, as explained xref:ug:fun:ugfun.adoc#simulating-collections-of-values[elsewhere].
