...
In the section xref:ug:fun:ugfun.adoc#referencing-domain-entities[above] it was explained how a view model DTO can transparent reference any "backing" entities; these references are converted to internal object identifiers.
...
The xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`] can be used to obtain the object identifiers
...
The `isis-xsd` profile generates the XSD using the xref:rg:mvn:rgmvn.adoc#xsd[`xsd` goal] of Isis' maven plugin:
