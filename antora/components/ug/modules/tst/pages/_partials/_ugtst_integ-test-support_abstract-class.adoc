...
This base class xref:ugtst.adoc#bootstrapping[bootstraps] the framework (caching the framework on a thread-local), and provides various utility methods and framework-provided services for use by your application's subclass tests.
...
The primary benefit over its predecessor, `IntegrationTestAbstract2` (discussed xref:ugtst.adoc#IntegrationTestAbstract2[below] is that it allows the test to be bootstrapped by passing in a `Module` rather than an `AppManifest`.
...
<2> sets up a JMock context (using Apache Isis' extension to JMock as described in xref:ug:tst:ugtst.adoc#jmock-extensions[JMock Extensions].
<3> standard JUnit rule for writing tests that throw exceptions
<4> to capture messages that require translation, as described in xref:ug:btb:ugbtb.adoc#i18n[i18 support].
