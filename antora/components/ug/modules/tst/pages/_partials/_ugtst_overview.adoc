...
The xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] domain service allows a test to wrap domain objects and thus to interact with said objects "as if" through the UI:
...
The solution that Apache Isis provides is a domain service called xref:rg:cms:rgcms.adoc#FixtureScripts[Fixture Scripts], that defines a pattern and supporting classes to help ensure that the "data setup" for your tests are reusable and maintainable over time.
