...
Here's an example of a feature from the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype]:
...
(Commented out) configuration for this is provided in the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype].
...
[TIP]
====
The xref:rg:svc:rgsvc.adoc#Scratchpad[`Scratchpad`] domain service is one way in which glue classes can pass state between each other.
