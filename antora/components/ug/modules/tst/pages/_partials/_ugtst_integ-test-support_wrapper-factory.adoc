...
The xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] service is responsible for wrapping a domain object in a dynamic proxy, of the same type as the object being proxied.
...
----
<1> inject xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] into this test
