...
Here's the one provided by the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype]:
...
[TIP]
====
If you use Eclipse IDE on Windows then xref:support:dg:dg.adoc#workaround-for-path-limits[note the importance] of the `persistence.xml` file to make DataNucleus enhancer work correctly.
====
