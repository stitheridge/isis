...
One scenario in particular where this performance issues can arise is if your entities implement the `java.lang.Comparable` interface, and you have used Apache Isis' xref:rg:cms:rgcms.adoc#ObjectContracts[`ObjectContracts`] utility class.
...
This change has been made to both the xref:ug:fun:ugfun.adoc#_ugfun_getting-started_helloworld-archetype[HelloWorld] and xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp] archetypes.
