...
The server will use the xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] to attempt to
transform the domain object into the requested `x-ro-domain-type`.  The whole process is discussed in more detail in
the xref:ug:vro:ugvro.adoc#architecture[architecture] chapter.
...
This support is discussed further in the xref:ug:vro:ugvro.adoc#simplified-representations[simplified representations]
chapter.
...
this feature one can load a grid of data in a single call.  (That said, the xref:ug:vro:ugvro.adoc#simplified-representations[simplified representations]
