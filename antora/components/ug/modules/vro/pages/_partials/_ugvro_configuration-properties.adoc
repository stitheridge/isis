...
`honorUiHints`
| `true`,`false` (`_false_`)
|A mechanism for reducing the number of round-trips by eagerly rendering collections; discussed xref:ug:vro:ugvro.adoc#honor-ui-hints[here].
...
xref:ug:vro:ugvro.adoc#simplified-representations[Apache Isis profile] for all resources, rather than simply the
...
If no configuration property is available, then the defaults is determined by the xref:rg:cfg:rgcfg.adoc#deployment-types[deployment type]: production mode disables pretty printing, while prototype mode enables it.
...
| `true`,`false` (`_false_`)
| See xref:ug:vro:ugvro.adoc#object-property-values-only[discussion] below.
|
...
xref:ug:vro:ugvro.adoc#apache-isis-profile[Apache Isis profile], then the special `$$ro`
