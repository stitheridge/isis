...
+
The xref:rg:fis:rgfis.adoc#RepresentationService[`RepresentationService`] is an SPI domain service (plugin-point)
...
xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`], each one handling a particular
...
xref:ug:vro:ugvro.adoc#simplified-representations[simplified representation] of the Apache Isis profile; an implementation that provides support for the `x-ro-domain-type` parameter, and a
default/fallback implementation that returns the representations defined by the RO spec.

* `ContentMappingService` +
+
The xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] is used by the implementation of
