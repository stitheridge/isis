...
As described xref:rg:cfg:rgcfg.adoc#configuration-files[here], by default Apache Isis itself bootstraps from the
...
This is intended to support deployment through Docker, as discussed xref:ug:btb:ugbtb.adoc#docker[next].
====
...
[TIP]
====
Subsequent sections explain how to externalize configuration for xref:ugbtb.adoc#_ugbtb_deployment_tomcat[Tomcat] and for xref:ugbtb.adoc#docker[Docker].
====
...
If using Apache Isis' xref:ug:sec:ugsec.adoc#configuring-isis-to-use-shiro[Shiro integration] for authentication and/or authorization, note that it reads from the `shiro.ini` configuration file.
...
The xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] uses link:http://www.javassist.org[Javassist] to create on-the-fly classes acting as a proxy.
...
See the guidance in the xref:ug:odn:ugodn.adoc#using-jndi-data-source[configuring datanucleus] section.