...
[TIP]
====
This http://isis.markmail.org/thread/472c3mrvcgnripst[thread] from the users mailing list (in Apr 2014) shows a typical customization (to enable per-instance security) (though note that xref:ug:btb:ugbtb.adoc#multi-tenancy[Multi-Tenancy] is now a better solution to that particular use-case.
====
