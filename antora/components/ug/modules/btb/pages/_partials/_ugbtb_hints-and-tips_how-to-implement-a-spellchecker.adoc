...
One way to implement is to use the xref:rg:svc:rgsvc.adoc#EventBusService[event bus]:

* Set up a xref:rg:cms:rgcms.adoc#_rgcms_classes_domainevent[domain event] xref:rg:cms:rgcms.adoc#AbstractSubscriber[subscriber] that can veto the changes.

* if the change is made through an action, you can use xref:rg:ant:rgant.adoc#domainEvent[`@Action#domainEvent()`].

if if the change is made through an edit, you can use xref:rg:ant:rgant.adoc#domainEvent[`@Property#domainEvent()`].
