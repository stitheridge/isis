...
One way to meet this requirement is using the framework's built-in xref:rg:ant:rgant.adoc#semantics[`@Action#semantics()`] attribute:
...
Note that the action itself does not use the boolean parameter, it is only used by the supporting xref:rg:cms:rgcms.adoc#validate[`validate...()`] method.