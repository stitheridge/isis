...
xref:rg:svc:rgsvc.adoc#IsisJdoSupport[IsisJdoSupport] domain service, then you are likely to confuse the
...
The framework provides the xref:rg:svc:rgsvc.adoc#MessageService[`MessageService`] as a means to return an out-of-band
...
To handle this the xref:rg:svc:rgsvc.adoc#ExceptionRecognizer[`ExceptionRecognizer`] SPI can be used.  The
