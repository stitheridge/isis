...
In practical terms, this means that view models should be instantiated using a constructor, and then injecting services (if required) using the xref:rg:svc:rgsvc.adoc#ServiceRegistry[`ServiceRegistry`] service:
...
What will most likely *fail* is to use the xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`]:
