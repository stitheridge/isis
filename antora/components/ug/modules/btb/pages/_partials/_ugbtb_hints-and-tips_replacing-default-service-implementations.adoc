...
The trick is to use the xref:rg:ant:rgant.adoc#menuOrder[`@DomainService#menuOrder()`] attribute, specifying a low number (typically `"1"`).
...
[NOTE]
====
It is also possible to use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] attribute, rather than xref:rg:ant:rgant.adoc#menuOrder[`@DomainService#menuOrder()`].
...
If both attributes are present, then the value of xref:rg:ant:rgant.adoc#menuOrder[`@DomainServiceLayout#menuOrder()`] is used.
====
