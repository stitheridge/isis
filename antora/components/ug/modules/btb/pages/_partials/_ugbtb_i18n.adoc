...
* if running in prototype mode xref:rg:cfg:rgcfg.adoc#deployment-types[deployment type] or during integration tests, then the service runs in *write* mode, in which case it records all translations into the `.pot` file.
The `.pot` file is written out when the system is shutdown.
* if running in server (production) mode xref:rg:cfg:rgcfg.adoc#deployment-types[deployment type], then the service runs in *read* mode.
...
Here's a (silly) example from the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype]:
...
The same class is used in xref:rg:svc:rgsvc.adoc#MessageService[`MessageService`] so that you can raise translatable info, warning and error messages; each of the relevant methods are overloaded.
...
Another mechanism by which messages can be rendered to the user are as the result of exception messages thrown and recognized by an xref:rg:svc:rgsvc.adoc#ExceptionRecognizer[`ExceptionRecognizer`].
...
In addition, there are a reasonably large number of messages that are used for both login and the
 xref:ug:vw:ugvw.adoc#user-registration[user registration] (self sign-up) and password reset features.
...
For this, we recommend that you ensure that all such methods are tested through an xref:ug:tst:ugtst.adoc#integ-test-support[integration test] (not unit test).
...
These are simply passed through to the registered xref:rg:svc:rgsvc.adoc#ExceptionRecognizer[`ExceptionRecognizer`]s so that any messages are recorded as requiring translation.
====
...
For this to work, the `namedEscaped()` attribute must be specified using either the xref:ug:vw:ugvw.adoc#_ugvw_layout_file-based[layout file], or using an annotation such as xref:rg:ant:rgant.adoc#rgant-ParameterLayout[`@ParameterLayout`].
...
However, Apache Isis can be configured to read config files from an xref:ug:btb:ugbtb.adoc#externalized-configuration[external directory]; this is also supported for translations.
