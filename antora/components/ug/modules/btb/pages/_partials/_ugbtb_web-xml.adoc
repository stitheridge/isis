...
See the xref:ug:sec:ugsec.adoc#configuring-isis-to-use-shiro[security chapter] for full details on this topic.
...
----
<1> alternatively set to "development"; see xref:rg:cfg:rgcfg.adoc#deployment-types[deployment types] for further discussion.
...
Typically they will be logon pages, or for health checks (as per xref:rg:svc:rgsvc.adoc#HealthCheckService[`HealthCheckService`] SPI.
