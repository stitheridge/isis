...
Apache Isis' has a pluggable architecture allowing different user interface technologies.
 The principal implementation (as configured by the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype]) is the xref:ug:vw:ugvw.adoc#[Wicket viewer].
