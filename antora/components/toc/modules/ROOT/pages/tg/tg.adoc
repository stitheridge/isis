...
Set up xref:support:dg:dg.adoc#ide[an IDE] and import the project to be able to run and debug the app.

Then set up a launch configuration and check that you can:

* Run the app from within the IDE
* Run the app in debug mode
* Run with different deploymentTypes; note whether prototype actions (those annotated xref:rg:ant:rgant.adoc#restrictTo[`@Action(restrictTo=PROTOTYPING`]) are available or not:
...
* specify a xref:ug:fun:ugfun.adoc#object-titles-and-icons[title]
* specify an xref:ug:fun:ugfun.adoc#object-titles-and-icons[icon]
* make the entity bookmarkable by adding the xref:rg:ant:rgant.adoc#bookmarking[`@DomainObjectLayout#bookmarking()`] attribute.
...
** as per the docs describing xref:ug:fun:ugfun.adoc#crud[how to create or delete objects]
* rename if you wish
** eg `newPet(...)` or `addPet(...)`
* review `listAll` action (acting as a repository)
* as per the docs describing xref:ug:fun:ugfun.adoc#crud[how to write a custom repository]
...
* use the injected xref:rg:svc:rgsvc.adoc#IsisJdoSupport[`IsisJdoSupport`] domain service.
...
* use the xref:rg:ant:rgant.adoc#named[`@ParameterLayout(named=...)`] annotation to specify the name of action parameters
* use the xref:rg:ant:rgant.adoc#semantics[`@Action(semanticsOf=...)`]  annotation to indicate the semantics of the action (safe/query-only, idempotent or non-idempotent)
* annotate safe action as bookmarkable using xref:rg:ant:rgant.adoc#bookmarking[`@ActionLayout(bookmarking=...)`]
* confirm is available from bookmark panel (top-left of Wicket UI)
...
* experiment changing xref:rg:ant:rgant.adoc#semantics[`@Action(semantics=...)`] on actions
...
* add some xref:ug:fun:ugfun.adoc#properties[value properties]; also:
* for string properties
** use the xref:rg:ant:rgant.adoc#multiLine[`@PropertyLayout(multiLine=...)`] annotation to render a text area instead of a text box
** use the xref:rg:ant:rgant.adoc#maxLength[`@Property(maxLength=...)`] annotation to specify the maximum number of characters allowable
** use joda date/time properties, bigdecimals and blob/clob properties
* use the xref:rg:ant:rgant.adoc#optionality[`@Column(allowsNull=...)`] annotation specify whether a property is optional or mandatory
...
** rather than the xref:rg:cms:rgcms.adoc#title[`title()`] method
* order the properties using the xref:rg:ant:rgant.adoc#_rgant-MemberOrder[`@MemberOrder`], also `@MemberGroupLayout`
** see also the docs on xref:ug:vw:ugvw.adoc#annotation-based[static layouts]
...
* add some xref:ug:fun:ugfun.adoc#properties[reference properties]
...
** use the xref:rg:ant:rgant.adoc#bounding[`@DomainObjectLayout(bounding=...)`] annotation on the referenced type if there are only a small number (bounded) of instances
** use a xref:rg:cms:rgcms.adoc#choices[`choices...()`] supporting method
*** on a property
*** on an action parameter
** use a xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete...()`] supporting method
*** on a property
*** on an action parameter
...
* Add xref:ug:fun:ugfun.adoc#drop-downs-and-defaults[defaults] for action parameters
...
** use the xref:rg:cms:rgcms.adoc#ObjectContracts[`ObjectContracts`] utility class to help implement `Comparable`
*** you can also implement `equals()`, `hashCode()`, `toString()`
* Add a xref:ug:fun:ugfun.adoc#collections[collection] to one of the entities
** Use `SortedSet` as the class
** Use the xref:rg:ant:rgant.adoc#render[`@CollectionLayout(render=...)`] annotation to indicate if the collection should be visible or hidden by default
* optional: use the xref:rg:ant:rgant.adoc#sortedBy[`@CollectionLayout(sortedBy=...)`] annotation to specify a different comparator than the natural ordering
...
* to create objects, xref:ug:fun:ugfun.adoc#inject-services[inject] associated domain service
...
* the service itself should use xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`]
...
* the bootstrap "btn" CSS classes can be used using the xref:rg:ant:rgant.adoc#cssClass[`@ActionLayout(cssClass=...)`] annotation

*  the http://fortawesome.github.io/Font-Awesome/icons/[Font Awesome] icons can be used using the xref:rg:ant:rgant.adoc#cssClassFa[`@ActionLayout(cssClassFa=...)`]

It's also possible to use Font Awesome icons for the xref:ug:fun:ugfun.adoc#object-titles-and-icons[domain object icon].
...
* Delete the various hint annotations and instead specify layout hints using a  xref:ug:vw:ugvw.adoc#file-based[.layout.xml] file.
...
* Use the xref:rg:ant:rgant.adoc#hidden[`Property(hidden=...)`] annotation to make properties invisible
** likewise xref:rg:ant:rgant.adoc#hidden[`@Collection(hidden=...)`] for collections
...
* Use the xref:rg:cms:rgcms.adoc#hide[`hide...()`] supporting method on properties, collections and actions to make a property/collection/action invisible according to some imperative rule


=== Use it!

* Use the xref:rg:ant:rgant.adoc#editing[`Property(editing=...)`] annotation to make property read-only
** likewise xref:rg:ant:rgant.adoc#editing[`@Collection(editing=...)`] for collections
** alternatively, use xref:rg:ant:rgant.adoc#editing[`@DomainObject(editing=...)`] to disable editing for all properties/collections
* Use the xref:rg:cms:rgcms.adoc#disable[`disable...()`] supporting method on properties and actions to make a property/action disabled according to some imperative rule


=== Do it!

* use the xref:rg:ant:rgant.adoc#_rgant-Property_regexPattern[`@Property(regexPattern=...)`] annotation to specify a regex pattern for properties, and use xref:rg:ant:rgant.adoc#regexPattern[`@Parameter(regexPattern=...)`] for parameters
* use the xref:rg:ant:rgant.adoc#_rgant-Property_maxLength[`@Property(maxLength=...)`] annotation to indicate a maxmum number of characters, and xref:rg:ant:rgant.adoc#maxLength[`@Parameter(maxLength=...)`] for parameters
* Use the xref:rg:cms:rgcms.adoc#validate[`validate...()`] supporting method on properties or action parameter
* optional: for any data type:
** use the xref:rg:ant:rgant.adoc#_rgant-Property_mustSatisfy[`Property(mustSatisfy=...)`] and xref:rg:ant:rgant.adoc#mustSatisfy[`Parameter(mustSatisfy=...)`] annotations to specify arbitrary constraints on properties and parameters
...
* inject xref:rg:svc:rgsvc.adoc#ClockService[`ClockService`]
...
** annotate with xref:rg:ant:rgant.adoc#ONLY)`]
...
* For this action:
** add the xref:rg:ant:rgant.adoc#contributedAs[`@ActionLayout(contributedAs=ASSOCIATION)`] annotation
...
By default the events generated are `ActionDomainEvent.Default` (for actions) and `PropertyDomainEvent.Default` (for properties). Subclasses of these can be specified using the xref:rg:ant:rgant.adoc#_rgant-Action_domainEvent[`@Action(domainEvent=...)`] or xref:rg:ant:rgant.adoc#domainEvent[`Property(domainEvent=...)`] for properties.


Using the guidance in the docs for the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`]:
...
Actions are specified as being bulk actions using the xref:rg:ant:rgant.adoc#COLLECTION)`] annotation.
...
* Inject the xref:rg:svc:rgsvc.adoc#ActionInvocationContext[`ActionInteractionContext`] (request-scoped) service
...
The similar xref:rg:svc:rgsvc.adoc#Scratchpad[`Scratchpad`] (request-scoped) domain service is a good way to share information between bulk action invocations:
...
The xref:rg:svc:rgsvc.adoc#QueryResultsCache[`QueryResultsCache`] (request-scoped) domain service allows arbitrary objects to be cached for the duration of a request.
...
Each element in the Wicket viewer (entity form, properties, collections, action button etc) is a component, each created by a internal API (`ComponentFactory`, described xref:ug:vw:ugvw.adoc#extending[here]).
...
To support these use cases, Apache Isis therefore allows you to write a view model, either by annotating the class with xref:rg:ant:rgant.adoc#_rgant-ViewModel[`@ViewModel`] or (for more control) by implementing the xref:rg:cms:rgcms.adoc#AbstractViewModel[`ViewModel`] interface.
...
https://code.google.com/p/mockito/[Mockito] seems to be the current favourite among Java developers for mocking libraries, but if you use JMock then you'll find we provide a `JUnitRuleMockery2` class and a number of other utility classes, documented xref:ug:tst:ugtst.adoc#unit-test-support[here].
...
Isis has great support for writing xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration tests]; well-written integration tests should leverage fixture scripts and use the xref:rg:svc:rgsvc.adoc#WrapperFactory[`@WrapperFactory`] domain service.
...
* as per xref:ug:vro:ugvro.adoc#configuration-properties[the documentation], configure the Restful Objects viewer to generate a simplified object representation: +
+
[source,ini]
----
