...
The method is _also_ called for properties in the case when an object is newly instantiated using xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_RepositoryService[`RepositoryService#instantiate(...)`] or xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService#instantiate(...)`].
...
Defaults are also supported (of course) for xref:ug:fun:ugfun.adoc#contributed-members[contributed actions].
...
The first is to use the xref:rg:cms:rgcms.adoc#_rgcms_methods_lifecycle_created[`created()`] callback, called by the framework when xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_RepositoryService[`RepositoryService#instantiate(...)`] or xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService#instantiate(...)`] is called.
