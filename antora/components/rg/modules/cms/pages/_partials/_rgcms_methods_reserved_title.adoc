...
Note that Apache Isis' xref:ug:btb:ugbtb.adoc#i18n[i18n support] extends this so that titles can also be internationalized.
...
----
<1> simple xref:rg:cms:rgcms.adoc#TitleBuffer[utility class] to help construct the title string
<2> imperative conditional logic
<3> using the xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] to determine if the `dueBy` field is visible for this user ...
<4> ... but ignore if not
...
* xref:rg:cms:rgcms.adoc#iconName[`iconName()`]
* xref:rg:cms:rgcms.adoc#cssClass[`cssClass()`]
* xref:rg:cms:rgcms.adoc#layout[`layout()`]
