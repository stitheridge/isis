...
The `choices...()` supporting method is called for both action parameters and for properties, to find objects from a drop-down list box.  Unlike xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete...()`], the use case is when the number of objects is comparatively small and can be selected from a drop-down without any additional filtering.
...
This functionality is actually implemented as a xref:ug:fun:ugfun.adoc#contributed-members[contributed action], so the code for this is:
