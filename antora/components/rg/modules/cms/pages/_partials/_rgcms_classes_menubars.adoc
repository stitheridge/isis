...
Similar to the xref:rgcms.adoc#layout[object layout] classes, the concrete menu classes support bootstrap3; support for other layout systems is possible.
...
This ability to serialize to/from XML is used by the xref:rg:svc:rgsvc.adoc#MenuBarsLoaderService[`MenuBarsLoaderService`], the default implementation of which reads the grid layout for a domain class from a `.layout.xml` file on the classpath.
...
This is similar to `ActionLayoutData` (of the xref:rgcms.adoc#layout[object layout] classes), however it also identifies the domain service to which it belongs.
