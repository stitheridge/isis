...
[TIP]
====
The framework-provided `AppManifestAbstract2` and `AppManifestAbstract2.Builder` makes it easy to write an `AppManifest`, defined by a set of xref:rgcms.adoc#Module[`Module`] implementations.
...
Each module is identified by a class; the framework simply uses that class' package as the root to search for domain services (annotated with xref:rg:ant:rgant.adoc#rgant-PersistenceCapable[`@PersistenceCapable`]).
...
[NOTE]
====
The recommendations in this section no longer apply if using the xref:rg:cms:rgcms.adoc#bootstrapping[`AppManifest2`] interface and ``Module``s to bootstrap your application.
====
...
+
Here, it's common to use the `org.apache.isis.WebServer` class to launch your application from the xref:ug:btb:ugbtb.adoc#cmd-line[command line].
...
+
The code to boostrap an integration test is shown in the xref:ug:tst:ugtst.adoc#bootstrapping[testing guide], but once again an `AppManifest` is required.
...
[IMPORTANT]
====
The framework-provided `AppManifestAbstract2` and `AppManifestAbstract2.Builder` supercede `AppManifestAbstract`, making it easy to write an `AppManifest` defined by a set of xref:rgcms.adoc#Module[`Module`] implementations.
====
...
For example, the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype]'s `AppManifest` is defined as:
