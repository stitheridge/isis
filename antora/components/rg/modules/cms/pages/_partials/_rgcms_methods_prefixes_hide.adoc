...
It's comparatively rare for properties or collections to be imperatively hidden from view, but actions are sometimes hidden or shown visible (as opposed to being just xref:rg:cms:rgcms.adoc#disable[disabled], ie greyed out).
...
Note that enabling this configuration property in effect means that xref:ug:fun:ugfun.adoc#_ugfun_building-blocks_types-of-domain-objects_mixins[mixins] must be used instead of xref:ug:fun:ugfun.adoc#contributions[contributed services] (because contributed actions are the one case where the value of a parameter to a supporting method may be non-null).
====
