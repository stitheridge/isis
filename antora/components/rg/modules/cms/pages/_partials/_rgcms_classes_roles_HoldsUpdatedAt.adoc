...
The current time is obtained from the xref:rg:svc:rgsvc.adoc#ClockService[`ClockService`].

Entities that implement this interface often also implement xref:rg:cms:rgcms.adoc#_rgcms_classes_roles_HoldsUpdatedBy[`HoldsUpdatedBy`] role interface; as a convenience the xref:rg:cms:rgcms.adoc#Timestampable[`Timestampable`] interface combines the two roles.
