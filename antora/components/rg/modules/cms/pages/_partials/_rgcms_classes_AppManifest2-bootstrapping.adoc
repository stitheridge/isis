...
The bootstrapping of an Apache Isis application has been modularized using the `Module` interface, with `AppManifest2` (an extension of the original xref:rgcms.adoc#AppManifest-bootstrapping[`AppManifest`] interface) defined in terms of ``Module``s also.
...
The `AppManifest2` interface extends xref:rgcms.adoc#api[`AppManifest`], additionally declaring the top-level `Module` that was used to create the app manifest, and fixtures for reference data setup and for teardown:
...
Additional `withXxx(...)` builder methods are inherited from xref:rgcms.adoc#bootstrapping[`AppManifestAbstract.Builder`].
...
<4> Optionally each `Module` can define a xref:rgcms.adoc#FixtureScript[`FixtureScript`] which holds immutable "reference data".
...
<5> Similarly, optionally each `Module` can define a tear-down xref:rgcms.adoc# entities (both reference data and operational/transactional data).
...
xref:rgcms.adoc#bootstrapping[Previously] this was done by using a single `AppManifest` for both the tests and the webapp.
...
For more on `IntegrationTestAbstract3`, see the xref:ug:tst:ugtst.adoc#bootstrapping[testing user guide].
