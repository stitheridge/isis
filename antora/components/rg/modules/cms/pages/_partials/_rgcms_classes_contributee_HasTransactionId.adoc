...
auditing entries or commands, or for xref:rg:svc:rgsvc.adoc#InteractionContext[``Interaction``]s persisted as published events.
...
Currently the only implementation of this interface is that for `PublishedObjects` applib interface as defined by the xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`].
