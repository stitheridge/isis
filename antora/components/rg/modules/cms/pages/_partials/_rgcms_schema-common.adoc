...
This is basically a formal XML equivalent to the `Bookmark` object obtained from the xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`].
...
With it, we have the ability to lookup any arbitrary object.  Further discussion and examples can be found xref:ug:fun:ugfun.adoc#oid[here].
...
<3> the object type, corresponding to either the xref:rg:ant:rgant.adoc#objectType[`@DomainObject#objectType()`] attribute, or to the (JDO)
xref:rg:ant:rgant.adoc#rgant-Discriminator[`@Discriminator`] annotation.
...
This is used by the xref:rg:cms:rgcms.adoc#cmd["cmd" schema] to represent the
...
* second, as references to the target of a command representing the _intention_ to invoke an action or edit a property, as described by the xref:rg:cms:rgcms.adoc#schema-cmd["cmd" (command)] schema.
+
...
* third, as references to the target of an interaction capturing the actual execution of an action invocation or property edit, as described by the xref:rg:cms:rgcms.adoc#schema-ixn["ixn" (interaction)] schema.
...
The common schema also defines a number of ancillary types, used either by the common schema itself (see above) or by the xref:rg:cms:rgcms.adoc#_rgcms_schema_cmd["cmd"] and xref:rg:cms:rgcms.adoc#ixn["ixn"] schemas.
...
Used by both the xref:rg:cms:rgcms.adoc#_rgcms_schema_cmd["cmd"] and xref:rg:cms:rgcms.adoc#ixn["ixn"] schemas.
