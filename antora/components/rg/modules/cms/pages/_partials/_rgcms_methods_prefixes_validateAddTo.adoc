...
where the returned string is the reason the collection modification invocation is vetoed (or `null` if not vetoed).  Apache Isis' xref:ug:btb:ugbtb.adoc#i18n[i18n support] extends this so that reasons can be internationalized if required.
...
See also xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_addTo[`addTo...()`] and  xref:rg:cms:rgcms.adoc#validateRemoveFrom[`validateRemoveFrom...()`]`
