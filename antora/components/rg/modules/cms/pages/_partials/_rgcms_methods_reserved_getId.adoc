...
* as the value of `Bookmark#getObjectType()` and in the `toString()` value of `Bookmark`
 (see xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`])
* in the serialization of `OidDto` in the xref:rg:cms:rgcms.adoc#_rgcms_schema-cmd[command] and xref:rg:cms:rgcms.adoc#schema-ixn[interaction] schemas
...
1. xref:rg:ant:rgant.adoc#objectType[`@DomainService#objectType()`]
2. xref:rg:cms:rgcms.adoc#getId[`getId()`]
