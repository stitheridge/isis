...
While we could write a separate xref:rg:cms:rgcms.adoc#hide[`hide...()`] method for each and every action, this could become painful.  So instead Isis allows a single `hide...(...)` method to be implemented that is applied to all members.
...
See also the similar method to xref:rg:cms:rgcms.adoc#disable[`disable()`] object members en-masse.
...
There is further discussion on this approach in xref:rg:cms:rgcms.adoc#disable[here].
