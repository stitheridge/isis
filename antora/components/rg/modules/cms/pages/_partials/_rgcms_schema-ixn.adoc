...
In fact, it actually defines a call-graph of such executions for those cases where the xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] is used to execute sub-actions/property edits.
...
<2> uses complex types defined in the xref:rg:cms:rgcms.adoc#_rgcms_schema_common["common" schema] and also the xref:rg:cms:rgcms.adoc#cmd["cmd" schema]
...
The transaction Id is used to correlate back to the xref:rg:cms:rgcms.adoc#_rgcms_schema_command[command] that represented the intention to perform this execution, as well as to any xref:rg:cms:rgcms.adoc#changes[changes] to domain objects that occur as a side-effect of the interaction.
...
It corresponds to the xref:rg:cms:rgcms.adoc#memberDto[`memberDto`] of the "cmd" schema; some elements are copied directly:
...
<3> the target object, corresponding to one of the elements of the `targets` element of the xref:rg:cms:rgcms.adoc#memberDto[`memberDto`]
<4> the member identifier; corresponds to `memberIdentifier` of the `member` element of the xref:rg:cms:rgcms.adoc#memberDto[`memberDto`]
<5> the _logical_ member identifier; corresponds to `logicalMemberIdentifier` of the `member` element of the xref:rg:cms:rgcms.adoc#memberDto[`memberDto`]
<6> the user executing the action invocation/property edit; corresponds to the `user` element of the xref:rg:cms:rgcms.adoc#memberDto[`memberDto`]
<7> the current "human-friendly" title of the target object
<8> the set of metrics captured for this execution, of type `metricsDto` defined xref:rg:cms:rgcms.adoc#ancillary[below].
<9> if the action invocation/property edit threw an exception, then this is captured here.
<10> if any sub-actions or sub-edits were performed via the xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`], then these are captured in the `childExecutions` element.
...
xref:rg:cms:rgcms.adoc#memberDto[`actionDto`] complex type of the "cmd" schema
...
It corresponds to the similar xref:rg:cms:rgcms.adoc#memberDto[`propertyDto`] complex type of the "cmd" schema
...
The xref:rg:cms:rgcms.adoc#changes[changes] schema also provides metrics on the number of objects loaded/changed, but relates to the entire interaction rather than just one (sub)execution of an interaction.