...
The component classes, meanwhile, are broadly equivalent to the "layout" annotations (xref:rg:ant:rgant.adoc#_rgant-PropertyLayout[`@PropertyLayout`], xref:rg:ant:rgant.adoc#_rgant-CollectionLayout[`@CollectionLayout`], xref:rg:ant:rgant.adoc#rgant-DomainObjectLayout[`@DomainObjectLayout`]
...
This ability to serialize to/from XML is used by the xref:rg:svc:rgsvc.adoc#GridLoaderService[`GridLoaderService`], the default implementation of which reads the grid layout for a domain class from a `.layout.xml` file on the classpath.
...
The xref:ug:vw:ugvw.adoc#tweaking-css-classes[`application.css`] file can then be used for application-specific CSS, allowing arbitrary fine-tuning of the layout of the page.
