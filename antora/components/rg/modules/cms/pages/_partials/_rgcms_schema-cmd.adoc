...
The transaction Id is used to correlate to the xref:rg:cms:rgcms.adoc#_rgcms_schema_interaction[interaction] that executes the command, and to any xref:rg:cms:rgcms.adoc#changes[changes] to domain objects occurring as a side-effect of that interaction.
...
The `paramsDto` type is defined xref:rg:cms:rgcms.adoc#ancillary[below].
...
Note also that there is a corresponding xref:rg:cms:rgcms.adoc#memberExecutionDto[`memberExecutionDto`]
...
xref:rg:cms:rgcms.adoc#schema-common["common"] schema.
