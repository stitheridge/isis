...
The `xsd` goal of the `isis-maven-plugin` uses the xref:rg:svc:rgsvc.adoc#JaxbService[`JaxbService`] to
generate XSD schemas from any JAXB-annotated xref:ug:fun:ugfun.adoc#view-models[view model/DTOs].
...
objects into ``OidDto``s (as defined by the Apache Isis xref:rg:cms:rgcms.adoc#schema-common[common schema]).
...
* `jaxbClasses` - a list of xref:ug:fun:ugfun.adoc#jaxb[JAXB-annotated view model] classes;
...
+
Defaults to `false`.  Most DTO classes will reference one another or the xref:rg:cms:rgcms.adoc#schema-common[common schema].  Normally it's fine to merge all these XSDs together.  This property, if set, results in each a separate directory for each generation of its XSD or XSDs.

* `commonSchemas` - (optional) whether to also generate the isis common schema(s). +
+
Defaults to `false`; if set then the call to xref:rg:svc:rgsvc.adoc#JaxbService[`JaxbService`] will set `IsisSchemas.INCLUDE` flag.
...
<2> XSD generation, to run the `xsd` goal and then assemble into a zip file; within a profile for modularity; see section xref:rgmvn.adoc#xsd-profile[below]
<3> XJC generation, to run the `xjc` to generate Java DTO classes from XSDs; within a profile for modularity; see section xref:rgmvn.adoc#xjc-profile[below]

The xref:rgmvn.adoc#__rgmvn_xsd_xsd-submodule_xsd-profile[sections] xref:rgmvn.adoc#xjc-profile[below] flesh out the gaps.
...
<4> catalog file indicates the location of the referenced xref:rg:cms:rgcms.adoc#schema-common[common schema] XSDs.
...
reference for the Apache Isis xref:rg:cms:rgcms.adoc#schema-common[common schema] is likely to be needed:
