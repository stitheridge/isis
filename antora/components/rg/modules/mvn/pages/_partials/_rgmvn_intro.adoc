...
* `swagger` +
+
Uses the xref:rg:svc:rgsvc.adoc#SwaggerService[`SwaggerService`] to generate link:http://swagger.io[Swagger] spec files that describe the public and/or private RESTful APIs exposed by the xref:ug:vro:ugvro.adoc#[RestfulObjects viewer].

* `xsd` +
+
Uses the xref:rg:svc:rgsvc.adoc#JaxbService[`JaxbService`] to generate XSD schemas from any JAXB-annotated view models/DTOs. +
+
This is instead of and preferable to using the JAXB link:https://jaxb.java.net/2.2.4/docs/schemagen.html[schemagen] tool, because it uses the framework's support (via xref:rg:ant:rgant.adoc#_rgant-XmlJavaTypeAdapter[`@XmlJavaTypeAdapter`]) to translate any references to domain objects into ``OidDto``s (as defined by the Apache Isis xref:rg:cms:rgcms.adoc#schema-common[common schema]).
...
All of these goals require an xref:rg:cms:rgcms.adoc#AppManifest-bootstrapping[`AppManifest`] to point the plugin at, so that it knows how to bootstrap an Isis runtime.
...
As noted in the introduction, all the goals require an xref:rg:cms:rgcms.adoc#AppManifest-bootstrapping[`AppManifest`] to point the plugin at, so that it knows how to bootstrap an Isis runtime.
...
For example, the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp]'s
manifest is:
...
The xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype] reflects these recommendations for the `validate` and `swagger` goals.
