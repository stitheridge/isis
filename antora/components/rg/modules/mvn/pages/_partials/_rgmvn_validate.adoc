...
Update the `pom.xml` (we recommend in your project's `dom` module, and with a xref:rg:mvn:rgmvn.adoc#app-manifest[minimal AppManifest]):
...
<3> the manifest discussed xref:rg:mvn:rgmvn.adoc#app-manifest[previously]; adjust as required
...
In the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp] application the `SimpleObject` defines an `updateName` action.
...
It is also possible to customize the validation, explained xref:ug:btb:ugbtb.adoc#custom-validator[here].
