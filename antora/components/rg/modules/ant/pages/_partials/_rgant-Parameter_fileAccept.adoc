...
The `fileAccept()` attribute applies only to xref:rg:cms:rgcms.adoc#Blob[`Blob`]
or xref:rg:cms:rgcms.adoc#Clob[`Clob`] parameters, indicating the type of file to accept when
uploading a new value.   The attribute is also supported on xref:rg:ant:rgant.adoc#fileAccept[properties].
