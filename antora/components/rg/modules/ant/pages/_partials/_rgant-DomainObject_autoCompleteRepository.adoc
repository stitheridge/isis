...
[TIP]
====
If you require additional control - for example restricting the returned results based on the object being interacted with - then use the xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete...()`] supporting method instead.
====
...
However, it *can* be hidden or placed on a domain service with xref:rg:ant:rgant.adoc#nature[nature] of `DOMAIN`, such that the action would not be rendered otherwise in the UI.
Also, the action cannot be xref:rg:ant:rgant.adoc#restrictTo[restricted to] prototyping only.
====
