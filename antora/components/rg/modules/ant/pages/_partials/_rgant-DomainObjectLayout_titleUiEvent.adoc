...
This is as an alternative to implementing xref:rg:cms:rgcms.adoc#rgant-Title[`@Title`] annotation, within the class itself.
(If either `title()` or `@Title` are present, then they will take precedence).

Subscribers subscribe through the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] and can
...
[NOTE]
====
The feature was originally introduced so that xref:rg:ant:rgant.adoc#_rgant-XmlRootElement[`@XmlRootElement`]-annotated xref:ug:fun:ugfun.adoc#view-models[view model]s could be kept as minimal as possible, just defining the data.
UI events allow subscribers to provide UI hints, while xref:ug:fun:ugfun.adoc#mixins[mixin]s can be used to provide the behaviour.
====
...
Subscribers (which must be domain services) subscribe using either the link:https://github.com/google/guava[Guava] API or (if the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] has been appropriately configured) using the link:http://www.axonframework.org/[Axon Framework] API.
...
However, events can be raised programmatically either by calling the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_EventBusService[`EventBusService`] API directly, or as a result of calling the xref:rg:svc:rgsvc.adoc#TitleService[`TitleService`]'s `titleOf(...)` method.


== See also

This attribute can also be applied to xref:rg:ant:rgant.adoc#titleUiEvent[view models].
