...
The `mustSatisfy()` attribute allows arbitrary validation to be applied to parameters using an (implementation of a) `org.apache.isis.applib.spec.Specification` object.  The attribute is also supported on xref:rg:ant:rgant.adoc#mustSatisfy[properties].

[TIP]
====
The specification implementations can (of course) be reused between parameters and xref:rg:ant:rgant.adoc#mustSatisfy[properties].
====
