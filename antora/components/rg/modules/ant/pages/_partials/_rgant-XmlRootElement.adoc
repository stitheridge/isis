...
xref:ug:fun:ugfun.adoc#view-models[view model], in particular one intended to act as a DTO for use within
...
In comparison to using either the xref:rg:cms:rgcms.adoc#ViewModel[`ViewModel`] interface or the
...
`oidDto` complex type (as defined by the Apache Isis xref:rg:cms:rgcms.adoc#schema-common[common schema]):
...
xref:rg:svc:rgsvc.adoc#UrlEncodingService[`UrlEncodingService`], the default implementation of which
...
xref:ug:btb:ugbtb.adoc#replacing-default-service-implementations[here] in the user guide.
====
...
Behaviour can therefore be provided using xref:ug:fun:ugfun.adoc#mixins[mixins] (annotated with
xref:rg:ant:rgant.adoc#_rgant-Mixin[`@Mixin`]), while xref:rg:cms:rgcms.adoc#uievent[UI events] can be used
to obtain title, icons and so on.

For a more complete discussion of writing JAXB view models/DTOs, see xref:ug:fun:ugfun.adoc#view-models[this topic]
in the user guide.
