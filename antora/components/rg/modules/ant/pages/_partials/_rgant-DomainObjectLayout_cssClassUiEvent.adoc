...
This is as an alternative to implementing xref:rg:cms:rgcms.adoc#cssClass[`cssClass()`] reserved method.
...
or (if the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] has been appropriately configured)
...
However, events can be raised programmatically either by calling the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] API directly.


== See also

This attribute can also be applied to xref:rg:ant:rgant.adoc#cssClassUiEvent[view models].
