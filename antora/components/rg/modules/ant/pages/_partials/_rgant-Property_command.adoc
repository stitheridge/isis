...
Each property edit is automatically reified by the xref:rg:svc:rgsvc.adoc#CommandContext[`CommandContext`] service into a `Command` object, capturing details of the target object, the property, the proposed new value fo the property, the user, a timestamp and so on.
...
For an example application, see xref:rgant.adoc#command[`Action#command()`].
