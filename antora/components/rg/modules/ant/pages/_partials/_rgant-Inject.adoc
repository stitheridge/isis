...
In fact, it also injects domain services into xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration tests] and xref:ug:tst:ugtst.adoc#fixture-scripts[fixture scripts].

[NOTE]
====
One omission: Apache Isis (currently) does not inject services into `o.a.i.applib.spec.Specification` instances (as used by xref:rg:ant:rgant.adoc#_rgant-Property_mustSatisfy[`@Property#mustSatisfy()`] and xref:rg:ant:rgant.adoc#mustSatisfy[`@Parameter#mustSatisfy()`] annotations.
====
...
----
<1> an alternative implementation would be to implement `findRecentOrders()` as a xref:ug:fun:ugfun.adoc#contributed-members[contributed action].
...
[NOTE]
====
An alternative and almost equivalent design would be to publish an event using the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] and implement the domain services as subscribers to the event.
...
It will also perform dependency injection if an object is created through the xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_FactoryService[`FactoryService`] or xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`].
...
If you prefer, though, you can simply instantiate domain objects using "new" and then inject domain services manually using xref:rg:svc:rgsvc.adoc#ServiceRegistry[`ServiceRegistry`]:
