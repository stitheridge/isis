...
These attributes can also be applied to xref:rg:ant:rgant.adoc#_rgant-DomainObjectLayout_cssClassFa[domain objects] and to xref:rg:ant:rgant.adoc#cssClassFa[view models] to specify the object's icon.
...
[NOTE]
====
The similar xref:rg:ant:rgant.adoc#cssClass[`@ActionLayout#cssClass()`] annotation attribute is also used as a hint to apply CSS, but for wrapping the representation of an object or object member so that it can be styled in an application-specific way.
====
