...
* first, the xref:rg:ant:rgant.adoc#menuBar[`menuBar()`] determines which of the three menu bars the service's actions should be rendered
* then, the domain service's top-level name (typically explicitly specified using xref:rg:ant:rgant.adoc#named[`named()`]) is used to determine the top-level menu item to be rendered on the menu bar
