...
The `publishing()` attribute determines whether and how a property edit is published via the registered implementation of xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`].
...
xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] `isis.services.publish.properties` is used to
