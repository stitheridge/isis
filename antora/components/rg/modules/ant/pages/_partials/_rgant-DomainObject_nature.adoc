...
+
Equivalent to annotating with xref:rg:ant:rgant.adoc#_rgant-Mixin[`@Mixin`].  For further discussion on using mixins, see xref:ug:fun:ugfun.adoc#mixins[mixins] in the user guide.
...
As such the domain object's class cannot be annotated with xref:rg:ant:rgant.adoc#_rgant-ViewModel[`@ViewModel`] or implement the xref:rg:cms:rgcms.adoc#ViewModel[`ViewModel`] interface.
...
These limitations do _not_ apply to xref:ug:fun:ugfun.adoc#jaxb[JAXB] view models.
