...
This is as an alternative to implementing xref:rg:cms:rgcms.adoc#iconName[`iconName()`] reserved method.
(If `iconName()` is present, then it will take precedence).

Subscribers subscribe through the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] and can use obtain a reference to the domain object from the event.
...
However, events can be raised programmatically by calling the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] API directly.


== See also

This attribute can also be applied to xref:rg:ant:rgant.adoc#iconNameUiEvent[view models].
