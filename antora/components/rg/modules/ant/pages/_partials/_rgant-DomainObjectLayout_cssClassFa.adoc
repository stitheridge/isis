...
If necessary the icon specified can be overridden by a particular object instance using the xref:rg:cms:rgcms.adoc#iconName[`iconName()`] method.
...
[TIP]
====
The similar xref:rg:ant:rgant.adoc#cssClass[`@DomainObjectLayout#cssClass()`] annotation attribute is also used as a hint to apply CSS, but for wrapping the representation of an object or object member so that it can be styled in an application-specific way.
====
...
* xref:rg:ant:rgant.adoc#cssClassFa[view models] to specify the object's icon, and
* xref:rg:ant:rgant.adoc#cssClassFa[actions] to specify an icon for the action's representation as a button or menu item.
