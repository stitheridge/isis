...
The `@Programmatic` annotation causes the method to be excluded completely from the Apache Isis metamodel. This means it won't appear in any UI, and it won't appear in any xref:rg:svc:rgsvc.adoc#_rgsvc_integration-api_MementoService[memento]s or xref:rg:svc:rgsvc.adoc#XmlSnapshotService[snapshot]s.
