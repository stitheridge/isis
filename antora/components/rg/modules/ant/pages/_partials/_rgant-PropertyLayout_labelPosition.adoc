...
The one exception is xref:rg:ant:rgant.adoc#multiLine[`multiLine()`] string properties, where the label defaults to `TOP` automatically (to provide as much real-estate for the multiline text field as possible).
...
Instead, a default can be specified using a xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] in `isis.properties`:
...
This attribute can also be specified for xref:rg:ant:rgant.adoc#labelPosition[parameters].
