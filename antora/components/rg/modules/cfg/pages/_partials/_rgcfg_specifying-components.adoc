...
This is done using an xref:rg:cms:rgcms.adoc#AppManifest[`AppManifest`], specified either programmatically or through the configuration properties.
...
See the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype] for details.
...
Viewers are specified by way of the filters and servlets in the xref:ug:btb:ugbtb.adoc#web-xml[`web.xml`] file; these are not bootstrapped by the framework, rather it is the other way around.
