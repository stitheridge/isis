...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#domainEvent[`@Action#domainEvent()`] is not specified (is set to `ActionDomainEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#domainEvent[`@Collection#domainEvent()`] is not specified (is set to `CollectionDomainEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#domainEvent[`@Property#domainEvent()`] is not specified (is set to `PropertyDomainEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#createdLifecycleEvent[`@DomainObject#createdLifecycleEvent()`] is not specified (is set to `ObjectCreatedEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#loadedLifecycleEvent[`@DomainObject#loadedLifecycleEvent()`] is not specified (is set to `ObjectLoadedEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#persistingLifecycleEvent[`@DomainObject#persistingLifecycleEvent()`] is not specified (is set to `ObjectPersistingEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#persistedLifecycleEvent[`@DomainObject#persistedLifecycleEvent()`] is not specified (is set to `ObjectPersistedEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#removingLifecycleEvent[`@DomainObject#removingLifecycleEvent()`] is not specified (is set to `ObjectRemovingEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#updatingLifecycleEvent[`@DomainObject#updatingLifecycleEvent()`] is not specified (is set to `ObjectUpdatingEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#updatedLifecycleEvent[`@DomainObject#updatedLifecycleEvent()`] is not specified (is set to `ObjectUpdatedEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#cssClassUiEvent[`@DomainObjectLayout#cssClassUiEvent()`] is not specified (is set to `CssClassUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#iconUiEvent[`@DomainObjectLayout#iconUiEvent()`] is not specified (is set to `IconUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#layoutUiEvent[`@DomainObjectLayout#layoutUiEvent()`] is not specified (is set to `LayoutUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#titleUiEvent[`@DomainObjectLayout#titleUiEvent()`] is not specified (is set to `TitleUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#cssClassUiEvent[`@ViewModelLayout#cssClassUiEvent()`] is not specified (is set to `CssClassUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#iconUiEvent[`@ViewModelLayout#iconUiEvent()`] is not specified (is set to `IconUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#layoutUiEvent[`@ViewModelLayout#layoutUiEvent()`] is not specified (is set to `LayoutUiEvent.Default`).
...
|`true`,`false` +
(`true`)
|Whether an event should be posted if xref:rg:ant:rgant.adoc#titleUiEvent[`@ViewModelLayout#titleUiEvent()`] is not specified (is set to `TitleUiEvent.Default`).
...
|`isis.services`
|`FQCN`,`FQCN2`,...
|NO LONGER REQUIRED; replaced by xref:rg:cms:rgcms.adoc#AppManifest[`AppManifest`].
...
| `all`, `none` +
(`all`)
|Whether the changed properties of objects should be automatically audited (for objects annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`].
...
|Whether action invocations should be automatically reified into commands (for actions annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`].  +
...
`properties`
| `all`, `none` (`none`)
|(Whether property edits should be automatically reified into commands (for properties annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`].  +
...
| `all`, `none` +
(`all`)
|Whether changed objects should be automatically published (for objects annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`].
...
`actions`
| `all`, `ignoreSafe`, `none` (`none`)
|Whether actions should be automatically published (for actions annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`]. +

|`isis.services.` +
`publish.` +
`properties`
| `all`, `none` (`none`)
|Whether properties should be automatically published (for properties annotated with xref:rg:ant:rgant.adoc#CONFIGURED)`]. +
...
See xref:ug:btb:ugbtb.adoc#custom-validator[Custom Validator] to learn more.
...
|`true`,`false` +
(`true`)
|Ensures that all xref:ug:fun:ugfun.adoc#jaxb[JAXB view model]s are not `abstract` (so can be instantiated).
...
|`true`,`false` +
(`true`)
| Ensures that all xref:ug:fun:ugfun.adoc#jaxb[JAXB view model]s are not inner classes (so can be instantiated).
...
|`true`,`false` +
(`false`)
| Ensures that all xref:ug:fun:ugfun.adoc#jaxb[JAXB view model]s have a `public` no-arg constructor. +
...
|`true`,`false` +
(`true`)
| Ensures that for all xref:ug:fun:ugfun.adoc#jaxb[JAXB view model]s with properties that reference persistent entities, that those entities are annotated with `@XmlJavaTypeAdapter`.
...
|`true`,`false` +
(`true`)
| Ensures that for all xref:ug:fun:ugfun.adoc#jaxb[JAXB view model]s with properties that are dates or times, that those properties are annotated with `@XmlJavaTypeAdapter`.
...
|`true`,`false` +
(`false`)
| When searching for  xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_disable[`disableXxx()`] or xref:rg:cms:rgcms.adoc#hide[`hideXxx()`] methods, whether to search only for the no-param version (or also for supporting methods that match the parameter types of the action). +
...
Note that this in effect means that xref:ug:fun:ugfun.adoc#_ugfun_building-blocks_types-of-domain-objects_mixins[mixins] must be used instead of xref:ug:fun:ugfun.adoc#contributions[contributed services].
...
See xref:ug:fun:ugfun.adoc#action-icons-and-css[UI hints] for more details.
...
See xref:ug:btb:ugbtb.adoc#finetuning[finetuning the programming model] for more details.
...
|`FQCN`,`FQCN2`,...
|Fully qualified class names of classes to be instantiated to read layout metadata, as used in for xref:ug:vw:ugvw.adoc#file-based[file-based layout]s. +

See xref:ug:btb:ugbtb.adoc#layout-metadata-reader[Layout Metadata Reader] for more information.
...
|`true`,`false` +
(`true`)
|Whether objects' properties and collections can be edited directly (for objects annotated with xref:rg:ant:rgant.adoc#_rgant-DomainObject_editing[`@DomainObject#editing()`]); see xref:rg:cfg:rgcfg.adoc#isis-objects-editing[below] for further discussion.
...
See xref:rg:cfg:rgcfg.adoc#filterVisibility[section below] for further discussion.
...
Filtering is supported by the xref:ug:vw:ugvw.adoc#[Wicket viewer] and the xref:ug:vro:ugvro.adoc#[Restful Objects viewer], and also by the xref:rg:svc:rgsvc.adoc# "skip rules").
