...
objects.  This functionality is surfaced through the user interface through a related xref:rg:svc:rgsvc.adoc#related-mixins-and-menus[mixin and menu action].
...
* the xref:rg:ant:rgant.adoc#Object[`Object` mixin] provides the ability to download the XML layout for
...
The xref:rg:svc:rgsvc.adoc#GridService[`GridService`] is responsible for loading and normalizing layout XML for
a domain class.  It in turn uses the xref:rg:svc:rgsvc.adoc#_rgsvc_presentation-layer-spi_GridLoaderService[`GridLoaderService`] and  xref:rg:svc:rgsvc.adoc#GridSystemService[`GridSystemService`]
services.