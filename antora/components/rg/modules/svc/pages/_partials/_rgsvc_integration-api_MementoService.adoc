...
The `MementoService` was originally introduced to simplify the implementation of xref:ug:fun:ugfun.adoc#view-models[ViewModel]s which are required by the framework to return string representation of all of their backing state, moreover which is safe for use within a URL.
This usage is deprecated; use xref:ug:fun:ugfun.adoc#jaxb[JAXB view models] instead.
...
To use an alternative implementation, implement `MementoService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#overriding-the-services[introduction] to this guide).
...
As noted in the introduction, a common use case for this service is in the implementation of the xref:rg:cms:rgcms.adoc#AbstractViewModel[`ViewModel`] interface.

[TIP]
====
Rather than implementing `ViewModel`, it's usually easier to annotate your view models with xref:rg:ant:rgant.adoc#_rgant-ViewModel[`@ViewModel`] (or equivalently xref:rg:ant:rgant.adoc#_rgant-DomainObject_nature[`@DomainObject#nature=EXTERNAL_ENTITY`] or xref:rg:ant:rgant.adoc#ENTITY`].
====
