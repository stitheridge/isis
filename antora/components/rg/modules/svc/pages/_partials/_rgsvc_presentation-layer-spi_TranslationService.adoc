...
[NOTE]
====
For the "big picture" and further details on Apache Isis' i18n support, see xref:ug:btb:ugbtb.adoc#i18n[here].
====
...
If the menu items are not required then these can be suppressed either using security or by implementing a xref:ug:btb:ugbtb.adoc#vetoing-visibility[vetoing subscriber].

For more details on the implementation, see xref:ug:btb:ugbtb.adoc#i18n[i18n support].
...
See xref:ug:btb:ugbtb.adoc#i18n[i18n support] to learn more about the translation service.
...
This service works in conjunction with xref:rg:svc:rgsvc.adoc#_rgsvc_presentation-layer-spi_LocaleProvider[`LocaleProvider`] and xref:rg:svc:rgsvc.adoc#TranslationsResolver[`TranslationsResolver`] in order to provide i18n support.
