...
The xref:rg:svc:rgsvc.adoc#BookmarkService[`BookmarkService`] provides a mechanism for obtaining a string representations of a single domain object.

The xref:rg:svc:rgsvc.adoc#MementoService[`MementoService`] also provides a mechanism for generating string representations of domain objects.

The xref:rg:svc:rgsvc.adoc#JaxbService[`JaxbService`] is a simple wrapper around
