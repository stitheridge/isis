...
The `SudoService` allows the current user reported by the xref:rg:svc:rgsvc.adoc#UserService[`UserService`]  to be temporarily changed to some other user.
This is useful both for xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration testing] (eg if testing a workflow system whereby objects are moved from one user to another) and while running xref:ug:tst:ugtst.adoc#fixture-scripts[fixture scripts] (eg setting up objects that would normally require several users to have acted upon the objects).
...
The current user/role reported by the internal xref:rg:fis:rgfis.adoc#AuthenticationSessionProvider[`AuthenticationSessionProvider`] will also return the specified user/roles.
...
See the xref:rg:svc:rgsvc.adoc#ACCESS-ALL-ROLE[`ACCESS-ALL-ROLE`] below for details of how to circumvent this.
====
...
When `sudo(...)` is called the "effective user" is reported by both xref:rg:svc:rgsvc.adoc#_rgsvc_core-domain-api_UserService[`UserService`] and by xref:rg:fis:rgfis.adoc#AuthenticationSessionProvider[`AuthenticationSessionProvider`], but does not propagate through to the xref:ug:sec:ugsec.adoc#[Shiro security mechanism].
...
For example if running a fixture script (which uses the xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_WrapperFactory[`WrapperFactory`]) from within an implementation of xref:rg:svc:rgsvc.adoc#UserRegistrationService[`UserRegistrationService`], this is likely to result in ``HiddenException``s being thrown because there is no effective user.
...
The default implementation of xref:rg:svc:rgsvc.adoc#UserService[`UserService`] has been refactored to leverage this SPI.
