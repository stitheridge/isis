...
* an object type of "custmgmt.Customer" (as per xref:rg:ant:rgant.adoc#objectType[`DomainObject#objectType()`] or equivalent) , and
* an id=123
...
(a polymorphic relationship).  For example, the (non-ASF) link:http://platform.incode.org[Incode Platform^]'s auditing module's implementation of xref:rg:svc:rgsvc.adoc#AuditerService[`AuditerService`] uses bookmarks to capture the object that is being audited.
...
To use an alternative implementation, implement `BookmarkService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#overriding-the-services[introduction] to this guide).
...
Bookmarks are used by the (non-ASF) link:http://platform.incode.org[Incode Platform^]'s command module's implementation of  xref:rg:svc:rgsvc.adoc#BackgroundCommandService[`BackgroundCommandService`], which uses a bookmark to capture the target object on which an action will be invoked subsequently.
...
xref:rg:svc:rgsvc.adoc#AuditerService[`AuditerService`].
