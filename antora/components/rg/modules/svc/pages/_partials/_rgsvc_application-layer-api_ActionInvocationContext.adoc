...
The `ActionInvocationContext` domain service is a xref:rg:ant:rgant.adoc#_rgant-RequestScoped[`@RequestScoped`] service intended to support the implementation of "bulk" actions annotated with xref:rg:ant:rgant.adoc#invokeOn[`@Action#invokeOn()`].
...
One idiom is for the domain objects to also use the xref:rg:svc:rgsvc.adoc#Scratchpad[`Scratchpad`] service to share information, for example to aggregate values.
