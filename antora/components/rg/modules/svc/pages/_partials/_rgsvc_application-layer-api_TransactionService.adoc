...
xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`]'s `persist()` or `delete()` methods), then the framework actually queues up the work and only performs the persistence command either at the end of the transaction or immediately prior to the next query.
...
xref:rg:cms:rgcms.adoc#HasTransactionId[`HasTransactionId`] mixin.
