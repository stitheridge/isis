...
[NOTE]
====
For a discussion of the use of the `WrapperFactory` within integration tests (the primary or at least original use case for this service) can be found xref:ug:tst:ugtst.adoc#wrapper-factory[here]
====
...
This capability is frequently used within xref:ug:tst:ugtst.adoc#integ-test-support[integration tests], but can also be used in production code. (There are analogies that can be drawn here with the way that JEE beans can interact through an EJB local interface).
...
If the interface is performed (action invoked or property set), then - irrespective of whether the business rules were checked or skipped -  a xref:rg:ant:rgant.adoc#_rgant-Action_command[command]s will be created and pre- and post-execute xref:rg:ant:rgant.adoc#domainEvent[domain event]s) will be fired
...
* the xref:rg:cms:rgcms.adoc#title[`title()`] and `toString()` methods
* any xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_default[`default...()`], xref:rg:cms:rgcms.adoc#_rgcms_methods_prefixes_choices[`choices...()`] or xref:rg:cms:rgcms.adoc#autoComplete[`autoComplete...()`] methods
...
[NOTE]
====
It ought to be possible to implement an xref:rg:svc:rgsvc.adoc#ExceptionRecognizer[`ExceptionRecognizer`]s that would allow the above boilerplate to be removed.
