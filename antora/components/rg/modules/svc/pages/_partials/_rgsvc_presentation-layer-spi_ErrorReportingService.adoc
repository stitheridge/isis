...
The service is responsible for translating this into the language of the end-user (it can use xref:rg:svc:rgsvc.adoc#LocaleProvider[`LocaleProvider`] if required).
<2> whether this message has already been recognized by an xref:rg:svc:rgsvc.adoc#ExceptionRecognizer[`ExceptionRecognizer`] service.
