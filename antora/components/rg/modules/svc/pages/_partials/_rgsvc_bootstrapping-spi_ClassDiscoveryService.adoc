...
This service is used by the xref:rg:cms:rgcms.adoc#_rgcms_classes_super_FixtureScripts[`FixtureScripts`] service to automatically locate any xref:rg:cms:rgcms.adoc#FixtureScript[`FixtureScript`] implementations.
...
To use an alternative implementation, implement the `ClassDiscoveryService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#overriding-the-services[introduction] to this guide).
...
The xref:rgsvc.adoc#FixtureScripts[`FixtureScripts`] domain service uses `ClassDiscoveryService` to discover ``FixtureScript``s implementations to present in the UI.
