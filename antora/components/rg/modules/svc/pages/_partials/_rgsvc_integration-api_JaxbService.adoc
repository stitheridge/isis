...
xref:rg:cms:rgcms.adoc#schema[common Isis schemas] (for example the `OidDto` class that represents a reference to
...
To use an alternative implementation, implement `JaxbService` interface and use xref:rg:ant:rgant.adoc#_rgant-DomainServiceLayout_menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained in the xref:rg:svc:rgsvc.adoc#overriding-the-services[introduction] to this guide).
...
xref:ug:fun:ugfun.adoc#view-models[view models].  The functionality to download XML and XSD schemas is also
exposed in the UI through mixins to xref:rg:cms:rgcms.adoc#Dto[`Dto`] interface.
