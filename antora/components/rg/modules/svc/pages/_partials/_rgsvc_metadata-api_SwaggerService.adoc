...
xref:ug:fun:ugfun.adoc#_ugfun_building-blocks_types-of-domain-objects_view-models[view model]s and to domain services with a xref:rg:ant:rgant.adoc#ONLY`.
...
<3> Generate a Swagger spec that is the same as private case (above), but also including any xref:rg:ant:rgant.adoc#restrictTo[prototype] actions.
...
Apache Isis' xref:rg:mvn:rgmvn.adoc#[Maven plugin] also provides a xref:rg:mvn:rgmvn.adoc#swagger[swagger goal] which allows the spec file(s) to be generated at build time.  this then allows client-side stubs can then be generated in turn as part of a build pipeline.
