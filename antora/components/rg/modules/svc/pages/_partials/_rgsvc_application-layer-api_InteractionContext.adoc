...
property.  If that top-level action or property uses xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`] to
...
xref:rg:ant:rgant.adoc#invokeOn[`@Action#invokeOn()`]), then this will result in multiple ``Interaction``s, one
...
* if auditing is configured (using xref:rg:svc:rgsvc.adoc#AuditerService[`AuditerService`]), they provide better audit information, since the
`Interaction.Execution` captures the 'cause' of an interaction and can be correlated to the audit records (the "effect"
of the interaction) by way of the xref:rg:cms:rgcms.adoc#HasTransactionId[`transactionId`]
...
----
<1> The unique identifier of this interaction.  This will be the same value as held in `Command` (obtainable from xref:rg:svc:rgsvc.adoc#CommandContext[`CommandContext`]).
...
<5> Generates numbers in a named sequence.  Used by the framework both to number successive interaction ``Execution``s and for events published by the xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`].
...
turn via the xref:rg:svc:rgsvc.adoc#WrapperFactory[`WrapperFactory`].
<7> The domain event fired via the xref:rg:svc:rgsvc.adoc#EventBusService[`EventBusService`] representing the
...
<10> A DTO (instance of the xref:rg:cms:rgcms.adoc#ixn["ixn" schema]) being a serializable representation of this action invocation/property edit.
...
Unlike the similar xref:rg:svc:rgsvc.adoc#_rgsvc_application-layer-api_CommandContext[`CommandContext`] domain service (discussed xref:rg:svc:rgsvc.adoc#Related-Classes[below]) there is no domain service to different implementations of `Interaction` to be used.
That said, the framework simply instantiates the `Interaction` using the
xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`].
If a different implementation of `Interaction` was required, then a custom implementation of
 xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`] could always be supplied.
====
...
xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`] SPI, and to emit domain events over the
...
This service is very similar in nature to xref:rg:svc:rgsvc.adoc#CommandContext[`CommandContext`], in that the
...
is annotated to run in the background (using xref:rg:ant:rgant.adoc#command[`@Action#commandExecuteIn()`], or
is explicitly created through the xref:rg:svc:rgsvc.adoc#BackgroundService[`BackgroundService`], then the actual
interaction/execution is deferred until some other mechanism invokes the command (eg as described
xref:ug:btb:ugbtb.adoc#BackgroundCommandExecution[here]).
