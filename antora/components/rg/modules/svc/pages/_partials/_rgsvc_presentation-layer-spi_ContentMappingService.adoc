...
The `ContentMappingService` supports the (default implementation of the) xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`] enabling the xref:ug:vro:ugvro.adoc#[RestfulObjects viewer] to represent domain objects in some other format as specified by the HTTP `Accept` header.

See xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`] for further discussion.
...
in the ordering defined by xref:rg:ant:rgant.adoc#menuOrder[`@DomainServiceLayout#menuOrder()`]).
...
The framework provides two implementations of this service, both in support of the xref:rg:svc:rgsvc.adoc#CommandService[`CommandService`] SPI.
...
The `CommandWithDto` interface is a subtype of `Command` for implementations that can be reified into xref:rg:cms:rgcms.adoc#schema-cmd[`CommandDto`] XML instances.
...
This service is a companion to the default implementation of the xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`].

The framework implementations of `ContentMappingService` use the xref:rgsvc.adoc#MetamodelService[`MetaModelService`] to lookup any custom implementations of `CommandDtoProcessor`.