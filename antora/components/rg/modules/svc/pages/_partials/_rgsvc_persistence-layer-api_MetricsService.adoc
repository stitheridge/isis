...
Corresponds to the number of times that the xref:rg:svc:rgsvc.adoc#AuditerService[`AuditerService`]'s `audit(...)` method will be called as the transaction completes.
...
The xref:rg:svc:rgsvc.adoc#PublisherService[`PublisherService`] also captures the metrics gathered by the `MetricsService` and publishes them as part of the `PublishedObjects` class (part of its SPI).
