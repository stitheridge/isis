...
If running in xref:rg:cfg:rgcfg.adoc#deployment-types[production] (server) mode, then the framework will (lazily) instantiate the ``SystemClock` when first required.
...
If running in xref:rg:cfg:rgcfg.adoc#deployment-types[prototype] mode, though, then the framework will instead instantiate `FixtureClock`.
