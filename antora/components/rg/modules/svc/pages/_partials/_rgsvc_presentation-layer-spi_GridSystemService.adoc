...
xref:rg:ant:rgant.adoc#menuOrder[`@DomainServiceLayout#menuOrder()`]).
...
the service with the lowest xref:rg:ant:rgant.adoc#menuOrder[`@DomainServiceLayout#menuOrder()`] takes precedence.
...
This service is used by xref:rg:svc:rgsvc.adoc#GridService[`GridService`].
