...
[NOTE]
====
As an alternative to performing content negotiation within the domain classes, the xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`] and
xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] SPI domain services allow the framework to perform the content negotiation responsibility.
====
...
xref:rg:ant:rgant.adoc#menuOrder[`@DomainServiceLayout#menuOrder()`] (as explained
in the xref:rg:svc:rgsvc.adoc#overriding-the-services[introduction] to this guide).
...
The REST request is submitted to a domain service with a xref:rg:ant:rgant.adoc#ONLY` (`MyRestApi` in the diagram).  This uses the `AcceptHeaderService` to obtain the values of the
...
this is used by the xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] to determine how to
