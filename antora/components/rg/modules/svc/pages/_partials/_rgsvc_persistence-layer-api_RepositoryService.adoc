...
As an alternative, you could also use link:http://www.datanucleus.org/products/accessplatform_4_0/jdo/jdoql_typesafe.html[JDO typesafe queries] through the xref:rg:svc:rgsvc.adoc#IsisJdoSupport[`IsisJdoSupport`] service.
...
xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`]'s `instantiate(...)` method, but is provided in the
...
However, this behaviour can be disabled using the  xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] `isis.services.container.disableAutoFlush`.
...
For more details, see xref:rg:svc:rgsvc.adoc#IsisJdoSupport[`IsisJdoSupport`].
====
...
The xref:rg:svc:rgsvc.adoc#FactoryService[`FactoryService`] is often used in conjunction with the `RepositoryService`, to instantiate domain objects before persisting.
