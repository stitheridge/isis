...
This allows the xref:rg:cms:rgcms.adoc#schema-cmd[`CommandDto`] memento to be extracted, from which the action can be invoked/property edited.
...
Behind the scenes this uses the xref:rgsvc.adoc#SudoService[`SudoService`].
<2> provided `Command` must implement `CommandDto`.

When the `Command` is executed, it will also be returned by xref:rgsvc.adoc#CommandContext[`CommandContext`].
