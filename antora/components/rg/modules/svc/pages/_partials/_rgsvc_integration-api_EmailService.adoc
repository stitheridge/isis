...
`port` +
| port number (`_587_`)
|The port number for the SMTP service on the the external SMTP host (used by xref:rg:svc:rgsvc.adoc#EmailService[`EmailService`]).  +
...
`sender.address` +
| email address
|The email address to use for sending out email (used by xref:rg:svc:rgsvc.adoc#EmailService[`EmailService`]).
*Mandatory*.  +
...
|The hostname of the external SMTP provider (used by xref:rg:svc:rgsvc.adoc#EmailService[`EmailService`]).  +
...
`sender.password` +
| email password
|The corresponding password for the email address to use for sending out email (used by xref:rg:svc:rgsvc.adoc#EmailService[`EmailService`]).
*Mandatory*.  +
...
|`true`,`false` +
(`true`)
|Whether to enable TLS for the email SMTP connection (used by xref:rg:svc:rgsvc.adoc#EmailService[`EmailService`]).  +
...
and these properties may optionally be configured (each has a default to use gmail, documented xref:rg:cfg:rgcfg.adoc#configuring-core[here]):

* `isis.service.email.sender.hostname`
* `isis.service.email.port`
* `isis.service.email.tls.enabled`

These configuration properties can be specified either in `isis.properties` or in an xref:ug:btb:ugbtb.adoc#_ugbtb_deployment_externalized-configuration[external configuration file], or programmatically using the xref:rg:cms:rgcms.adoc#AppManifest-bootstrapping[`AppManifest`].
...
This is because `EmailService` is used as part of the xref:ug:vw:ugvw.adoc#user-registration[user registration] functionality and is used by Wicket pages that are accessed outside of the usual Apache Isis runtime.
...
The email service is used by the xref:rg:svc:rgsvc.adoc#_rgsvc_presentation-layer-spi_EmailNotificationService[`EmailNotificationService`] which is, in turn, used by xref:rg:svc:rgsvc.adoc#UserRegistrationService[`UserRegistrationService`].