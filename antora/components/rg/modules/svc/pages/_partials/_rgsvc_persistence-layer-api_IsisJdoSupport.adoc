...
In general these act at a lower-level of abstraction than the APIs normally used (specifically, those of xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`]), but nevertheless deal with some of the most common use cases.
...
[TIP]
====
As an alternative, consider using DataNucleus' link:http://www.datanucleus.org/products/accessplatform_4_0/jdo/jdoql_typesafe.html[type-safe JDO query API], discussed xref:rg:svc:rgsvc.adoc#type-safe-query-api[below].
====
...
Using Eclipse IDE you may need to configure annotation processing manually; see the xref:support:dg:dg.adoc#enable-annotation-processing[Developers' Guide].
...
[NOTE]
====
You can find the full example of the JDOQL equivalent in the xref:rg:svc:rgsvc.adoc#RepositoryService[`RepositoryService`]
====
...
When writing xref:ug:tst:ugtst.adoc#_ugtst_integ-test-support[integration tests] you'll usually need to tear down some/all mutable transactional data before each test.  One way to do that is to use the `executeUpdate(...)` method described xref:rg:svc:rgsvc.adoc#executing-sql[above].
