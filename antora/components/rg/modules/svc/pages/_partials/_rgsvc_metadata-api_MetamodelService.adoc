...
<8> obtain an implementation of `CommandDtoProcessor` (if any) as per an xref:rg:ant:rgant.adoc#_rgant-Action_command[`@Action#commandDtoProcessor()`] or xref:rg:ant:rgant.adoc#command[`@Property#commandDtoProcessor()`].
+
This is used by the xref:rgsvc.adoc#implementations[framework-provided implementations] of `ContentMappingService`.
