...
The `MenuBarsLoaderService` is used by the default implementation of xref:rgsvc.adoc#_rgsvc_presentation-layer-spi_MenuBarsService[MenuBarsService] to return a a xref:rg:cms:rgcms.adoc#menubars[`MenuBars`] instance deserialized from the `menubars.layout.xml` file read from the classpath.
...
If not, then the xref:rgsvc.adoc#MenuBarsService[MenuBarsService] will cache the layout once loaded.
