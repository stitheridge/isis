...
This is then used by various other  (internal) domain services, notably xref:rg:fis:rgfis.adoc#_rgfis_persistence-layer_AuditingServiceInternal[`AuditingServiceInternal`] and xref:rg:fis:rgfis.adoc#PublishingServiceInternal[`PublishingServiceInternal`].
...
<2> Used by the framework to determine whether to set the "persist hint" on the `Command` object (as per xref:rg:svc:rgsvc.adoc#CommandContext[`CommandContext`]).
<3> Used by xref:rg:fis:rgfis.adoc#PublishingServiceInternal[`PublishingServiceInternal`] to obtain details of and
counters of all objects changed within the transaction.
<4> Used by xref:rg:fis:rgfis.adoc#AuditingServiceInternal[`AuditingServiceInternal`] to obtain all pairs of
...
Both the xref:rg:fis:rgfis.adoc#_rgfis_persistence-layer_AuditingServiceInternal[`AuditingServiceInternal`] and xref:rg:fis:rgfis.adoc#PublishingServiceInternal[`PublishingServiceInternal`] (internal) domain services query this object.
