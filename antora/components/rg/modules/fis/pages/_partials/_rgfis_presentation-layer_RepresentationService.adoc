...
However, it also delegates to the xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`] which provides a mechanism for altering representations according to the HTTP `Accept` header.
...
In all there are three domain services that can influence the representations generated: this service, xref:rg:fis:rgfis.adoc#_rgfis_presentation-layer_ContentNegotiationService[`ContentNegotiationService`] and the xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`].
...
This delegates to xref:rg:fis:rgfis.adoc#ContentNegotiationService[`ContentNegotiationService`] to generate an alternative representation; but if none is provided then it falls back on generating the representations as defined in the link:http://restfulobjects.org[Restful Objects spec] v1.0.
...
The default implementation delegates to xref:rg:fis:rgfis.adoc#_rgfis_presentation-layer_ContentNegotiationService[`ContentNegotiationService`], whose default implementation may delegate in turn to xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] (if present).