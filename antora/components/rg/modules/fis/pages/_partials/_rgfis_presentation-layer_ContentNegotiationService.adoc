...
* first is to solve the problem at the domain layer by defining a regular Apache Isis xref:ug:fun:ugfun.adoc#view-models[view model].  This is then surfaced over the RO viewer.  +
+
...
* a second option is to solve the problem at the persistence layer, but defining a (SQL) view in the database and then xref:ug:odn:ugodn.adoc#mapping-to-a-view[mapping this] to a (read-only) entity.  Again this is surfaced by the RO viewer.  +
+
...
+
The Apache Isis framework provides three implementations of `ContentNegotiationService` which inspects different elements of the HTTP `Accept` header.  One of these implementations, `ContentNegotiationServiceXRoDomainType` will further delegate down to the companion xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] service (if configured/available), based on the value of the "x-ro-domain-type" parameter of the header. +
+
...
The value of the `x-ro-domain-type` parameter corresponds to the DTO to be mapped into by the xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`].
...
+
which returns xref:ug:vro:ugvro.adoc#simplified-representations[simplified representations]
...
The default `ContentNegotiationServiceXRoDomainType` implementation provides a xref:rg:cfg:rgcfg.adoc#configuring-core[configuration property] which controls whether a mapped domain object is pretty-printed (formatted, indented) or not:
...
If the property is not set, then the default depends on the xref:rg:cfg:rgcfg.adoc#deployment-types[deployment type]; production mode will disable pretty printing, while prototyping mode will enable it.
...
The default implementation of `ContentNegotiationService` delegates to xref:rg:svc:rgsvc.adoc#ContentMappingService[`ContentMappingService`] (if present) to convert domain entities into a stable form (eg DTO).

The `ContentNegotiationService` is itself called by the (default implementation of) xref:rg:fis:rgfis.adoc#RepresentationService[`RepresentationService`].