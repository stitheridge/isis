...
<2> called by the framework when editing a property, to create a DTO (for the xref:rg:cms:rgcms.adoc#schema-ixn["ixn" schema]) capturing the details of the property edit (target, new value etc).
...
xref:rg:fis:rgfis.adoc#CommandDtoServiceInternal[`CommandDtoServiceInternal`], used to create the `CommandDto`
(from the xref:rg:cms:rgcms.adoc#schema-cmd["cmd" schema]).
