...
//* `ad` : for Asciidoc documentation; a full list can be found in the xref:support:dg:dg.adoc#asciidoc-templates[appendix].
...
for frameworks such as xref:support:dg:dg.adoc#project-lombok[Project Lombok].
...
then make sure you have correctly configured the xref:support:dg:dg.adoc#other-settings-compiler[annotation processor]
...
Running an Apache Isis application requires that the DataNucleus enhancer runs on the compiled bytecode.  As described xref:support:dg:dg.adoc#the-app[above], the recommended way to do this with IntelliJ is to use a Run configuration that runs the enhancer goal prior to launch.
...
Similarly, gradle can be run to reduce the turn-around time when tweaking the UI (defined by the
 xref:ug:vw:ugvw.adoc#file-based[`*.layout.xml`] file for each domain class), when the app is running.
