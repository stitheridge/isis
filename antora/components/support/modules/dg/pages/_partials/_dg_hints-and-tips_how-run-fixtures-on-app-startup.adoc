...
The standard approach is to use xref:ug:tst:ugtst.adoc#fixture-scripts[fixture scripts].
These can be run in on start-up typically by being specified in the xref:rg:cms:rgcms.adoc#_rgcms_classes_AppManifest-bootstrapping[`AppManifest`], see for example the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype].
