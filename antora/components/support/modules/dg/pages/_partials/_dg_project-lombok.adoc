...
so that it can emit additional bytecode (eg for the getter and setter).  See xref:support:dg:dg.adoc#other-settings-compiler[here] for details of setting up in IntelliJ (Eclipse has very similar support).

Apache Isis supports link:https://projectlombok.org/[Project Lombok], in that the annotations that would normally be placed on the getter (namely xref:rg:ant:rgant.adoc#_rgant-Property[`Property`], xref:rg:ant:rgant.adoc#_rgant-PropertyLayout[`@PropertyLayout`], xref:rg:ant:rgant.adoc#_rgant-Collection[`@Collection`], xref:rg:ant:rgant.adoc#rgant-MemberOrder[`@MemberOrder`]) can be placed on the field instead.
...
In the future we might extend/fork Lombok so that it understands Isis' own annotations (ie xref:rg:ant:rgant.adoc#rgant-Collection[`@Collection`])
