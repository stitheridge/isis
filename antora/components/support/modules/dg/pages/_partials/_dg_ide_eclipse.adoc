...
We provide a set of editor templates.  These are used to add new methods to existing classes.  (These are equivalent to the xref:support:dg:dg.adoc#live-templates[IntelliJ live templates]):
...
The xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp archetype] uses link:http://projectlombok.org[Project Lombok] annotations (`@Getter` and `@Setter` and so on) to reduce the boilerplate.
...
See the section in xref:dg.adoc#datanucleus-enhancer[DataNucleus enhancer] for details of the contents of the `pom.xml`.
Chances are it is already set up from running the xref:ug:fun:ugfun.adoc#_ugfun_getting-started_helloworld-archetype[HelloWorld] or the xref:ug:fun:ugfun.adoc#simpleapp-archetype[SimpleApp] archetype.
...
For details of setting up DCEVM, see the xref:support:dg:dg.adoc#dcevm[corresponding section] in the IntelliJ documentation.
