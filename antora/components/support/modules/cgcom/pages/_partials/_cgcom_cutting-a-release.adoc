...
* the release manager performing post-release tasks, for either a xref:support:cgcom:cgcom.adoc#_cgcom_post-release-successful[successful] or an xref:support:cgcom:cgcom.adoc#post-release-unsuccessful[unsuccessful] vote.
...
The subsequent sections describe how other committers can xref:support:cgcom:cgcom.adoc#_cgcom_verifying-releases[verify a release] and how the release manager can then perform xref:support:cgcom:cgcom.adoc#post-release[post-release] activities and set up for the next development iteration.

If you've not performed a release before, then note that there are some configuration xref:support:cgcom:cgcom.adoc#release-process-prereqs[prerequisites] that must be configured first.
...
This should be in line with our xref:support:cgcom:cgcom.adoc#versioning-policy[semantic versioning policy].
...
(One should exist already, xref:support:cgcom:cgcom.adoc#create-new-jira[created at] the beginning of the development cycle now completing).
<3> adjust as required (Tomcat is used to smoke test the simpleapp archetype)

Obviously, alter ``$ISISREL`` as required, and bump ``$ISISRC`` for re-releasing following an xref:support:cgcom:cgcom.adoc#post-release-unsuccessful[unsuccessful] release.
...
After release, there is a xref:cgcom.adoc#reset-revision[similar step at the end] to reset back to `1.0.0-SNAPSHOT`.
...
In this section we release `simpleapp`, in the following xref:cgcom.adoc#releasing-the-helloworld-archetype[section] we release `helloworld`.
...
[NOTE]
====
Unfortunately, Nexus does not seem to allow subkeys to be used for signing. See xref:support:cgcom:cgcom.adoc#key-generation[Key Generation] for more details.
====
...
Note that the email also references the procedure for other committers to xref:support:cgcom:cgcom.adoc#verifying-releases[verify the release].
