...
This section (appendix) describes the prerequisites for the xref:support:cgcom:cgcom.adoc#release-process[release process].
...
Ensure that Java 7 is installed and the toolchains plugin is configured, as described in the xref:support:dg:dg.adoc#configure-maven-toolchains-plugin[contributors' guide].
...
See the page on xref:support:cgcom:cgcom.adoc#key-generation[key generation] for more details.
