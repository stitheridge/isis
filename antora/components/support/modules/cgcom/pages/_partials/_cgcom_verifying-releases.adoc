...
Whenever a release manager announces a vote on a release (as per the xref:support:cgcom:cgcom.adoc#release-process[release process]) on the xref:toc:ROOT:support.adoc#[dev mailing list], it is the responsibility of the project's PMC to cast their vote on the release.
...
The main release auditing tool, http://creadur.apache.org/rat[Apache RAT] is used in the xref:support:cgcom:cgcom.adoc#cutting-a-release[release process].
